// Package rustscan wraps the rustscan container for fast port discovery.
package rustscan
...
var log = logger.WithContext("RustScan")
...
func (s *Service) Scan(ctx context.Context, target, ports string) ([]domain.PortEntry, string, error) {
	...
	log.Info("starting", logger.F("target", target), logger.F("ports", ports))
	...
	log.Info("complete", logger.F("target", target), logger.F("open", len(entries)))
