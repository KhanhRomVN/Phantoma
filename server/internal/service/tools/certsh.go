...
const certshDefaultTimeout = 30 * time.Second
const certshDefaultMatch = "LIKE"
...
// CertshService implements certificate search via crt.sh
type CertshService struct {
	client  *http.Client
	proxy   string
	match   string
	baseURL string
...
// NewCertshService creates a new crt.sh service
func NewCertshService() *CertshService {
	return &CertshService{
		client: &http.Client{
			Timeout: certshDefaultTimeout,
		},
		match:   certshDefaultMatch,
		baseURL: "https://crt.sh",
	}
}

// SetProxy configures HTTP proxy for the service
func (s *CertshService) SetProxy(proxyURL string) error {
	if proxyURL == "" {
		return nil
	}
...
func (s *CertshService) SetMatch(match string) {
	s.match = match
}
...
func (s *CertshService) Scan(ctx context.Context, req domain.ScanRequest) (domain.ScanResult, error) {
	if req.Target == "" {
		return domain.ScanResult{}, domain.ErrInvalidTarget
	}

	ctx, cancel := context.WithTimeout(ctx, certshDefaultTimeout)
	defer cancel()
...
func (s *CertshService) LiveCertificate(ctx context.Context, targetDomain string) (*Certificate, error) {
	if targetDomain == "" {
	...
	dialer := &tls.Dialer{
		...
		NetDialer: &net.Dialer{Timeout: certshDefaultTimeout},
	}
...
func (s *CertshService) fetchFeed(ctx context.Context, feedURL string) (*AtomFeed, error) {
...
func (s *CertshService) processFeed(feed *AtomFeed) []Certificate {
