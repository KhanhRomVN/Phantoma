...
const nucleiDefaultTimeout = 10 * time.Minute

var nucleiLog = logger.WithContext("Nuclei")

type NucleiService struct {
	container string
}

func NewNucleiService(container string) *NucleiService {
	return &NucleiService{container: container}
}

// Scan runs nuclei against the target with given tags (e.g. ["cve","exposed","misconfig"]).
func (s *NucleiService) Scan(ctx context.Context, target string, tags []string) ([]domain.VulnEntry, string, error) {
	ctx, cancel := context.WithTimeout(ctx, nucleiDefaultTimeout)
	defer cancel()
	...
	nucleiLog.Info("starting", logger.F("target", target), logger.F("tags", tags))
	result, execErr := dockerpkg.Exec(ctx, s.container, args...)

	vulns := parseNucleiJSONL(result.Stdout)
	nucleiLog.Info("complete", logger.F("target", target), logger.F("findings", len(vulns)))
