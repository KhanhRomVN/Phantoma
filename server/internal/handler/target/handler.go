...
// Handler xử lý HTTP requests cho targets.
type Handler struct {
	service *targetSvc.Service
}

// NewHandler tạo Target handler mới.
func NewHandler(svc *targetSvc.Service) *Handler {
...
func (h *Handler) Create(w http.ResponseWriter, r *http.Request) {
	var input domaintargets.CreateTargetInput
...
func (h *Handler) Update(w http.ResponseWriter, r *http.Request) {
	...
	var input domaintargets.UpdateTargetInput
	if err := json.NewDecoder(r.Body).Decode(&input); err != nil {
		response.Error(w, http.StatusBadRequest, "invalid request body: "+err.Error())
		return
	}
