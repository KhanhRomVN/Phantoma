...
import { RawDataView } from '../shared/RawDataView';
import { TimelineCluster } from '../shared/TimelineCluster';
...
export default function PersonRecon({ initialEmail = 'phantoma@gmail.com' }: PersonReconProps) {
  ...
  // Load data when active email changes
  useEffect(() => {
    const cached = DATA_CACHE[activeEmail];
    if (cached) {
      loadData(cached);
      return;
    }
    if (activeEmail === 'phantoma@gmail.com') {
      import('../../data/person/phantoma@gmail.com.json')
        ...
        .catch((err) => console.error('Failed to load Person mock data:', err));
    }
  }, [activeEmail, loadData]);
  ...
  const addEmail = useCallback(() => {
    ...
    if (!email) return;
    if (sessions.some((s) => s.email === email)) return;
    const sess: PersonSession = {
      id: `sess-person-${Date.now()}`,
      email,
      status: 'queued',
      progress: 0,
    };
  ...
  const renderContent = () => {
    ...
    if (!result) {
      return (
        <div className="flex-1 flex items-center justify-center flex-col gap-3 bg-[#0f1319]">
          ...
          <div className="text-[12px] font-mono text-[#c8d6f0]">
            Select an email from the left panel to view reconnaissance data
          </div>
        </div>
      );
    }
    ...
    switch (activeTab) {
      ...
      case 'identity':
        return <Identity dataPoints={displayDataPoints} activeGroup={activeGroup!} />;

      case 'contacts':
        return <Contacts dataPoints={displayDataPoints} activeGroup={activeGroup!} />;

      case 'social':
        return <SocialMedia dataPoints={displayDataPoints} activeGroup={activeGroup!} />;

      case 'technical':
        return <Technical dataPoints={displayDataPoints} activeGroup={activeGroup!} />;

      case 'leaks':
        return <Leaks dataPoints={displayDataPoints} activeGroup={activeGroup!} />;

      case 'services':
        return <Services dataPoints={displayDataPoints} activeGroup={activeGroup!} />;

      case 'noise':
        return <Noise dataPoints={displayDataPoints} activeGroup={activeGroup!} />;
      ...
      <div className="w-[293px] bg-[#0f1319] border-r border-[#1c2333] flex flex-col shrink-0 overflow-hidden">
        ...
        <div className="overflow-y-auto p-1.5 space-y-1 shrink-0 max-h-[35%]">
          {sessions.map((sess) => {
            ...
            return (
              <div
                key={sess.id}
                onClick={() => setActiveEmail(sess.email)}
              ...
              >
                ...
                {/* Row 1: Status dot + Email + Risk score */}
        ...
        {contextMenu && (
          ...
          >
            ...
            <button
              onClick={() => {
                removeEmail(contextMenu.sessionId);
                setContextMenu(null);
              }}
