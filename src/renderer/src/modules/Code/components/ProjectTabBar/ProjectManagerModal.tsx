    ...
    >
      <div className="bg-card-background border border-border rounded-xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-divider shrink-0">
          ...
          <button
            onClick={onClose}
            className="p-1 rounded hover:bg-sidebar-item-hover text-text-secondary/60 hover:text-text-secondary"
          >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {!showCreate ? (
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-3 border border-dashed border-border rounded-lg hover:border-primary hover:bg-primary/5 transition-colors text-text-secondary/60 hover:text-text-secondary"
            >
            ...
            <div className="bg-sidebar-background border border-border rounded-lg p-4 space-y-3">
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-input-background border border-border rounded-lg text-sm text-text-primary placeholder:text-text-secondary/40 outline-none focus:border-primary"
                autoFocus
              />
              <div>
                ...
                <div className="grid grid-cols-4 gap-2 mt-1.5">
                  {TEMPLATES.map((tpl) => (
                    <button
                      ...
                      className={cn(
                        'flex flex-col items-center gap-1 py-2 px-1 rounded-lg border transition-colors text-xs',
                        selectedTemplate === tpl.id
                          ? 'border-primary bg-primary/10 text-text-primary'
                          : 'border-border hover:border-border/60 text-text-secondary/60 hover:text-text-secondary',
                      )}
                    >
              ...
              <button
                ...
                className="w-full py-2 bg-primary rounded-lg text-sm font-medium text-text-foreground disabled:opacity-40 disabled:cursor-not-allowed hover:bg-primary/80"
              >
                Create Project
              </button>
            </div>
          ...
          <div className="space-y-1.5">
            {projects.length === 0 ? (
              ...
              projects.map((project) => (
                <div
                  key={project.id}
                  className="flex items-center gap-3 px-3 py-2.5 bg-sidebar-background border border-border rounded-lg hover:border-border/60 transition-colors group"
                >
                  <span
                  ...
                  {editingId === project.id ? (
                    <input
                      ...
                      className="flex-1 px-2 py-0.5 bg-input-background border border-border rounded text-sm text-text-primary outline-none focus:border-primary"
                      autoFocus
                    />
                  ) : (
                    <span className="flex-1 text-sm text-text-primary">{project.name}</span>
                  )}
                  <span className="text-[10px] text-text-secondary/40 px-2 py-0.5 bg-sidebar-item-hover rounded">
                    {project.template}
                  </span>
                  ...
                  <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      ...
                      className="p-1 rounded hover:bg-sidebar-item-hover text-text-secondary/40 hover:text-text-secondary"
                    >
                      <Edit2 className="w-3.5 h-3.5" strokeWidth={1.5} />
                    </button>
                    <button
                      onClick={() => handleDeleteProject(project.id)}
                      className="p-1 rounded hover:bg-error/20 text-text-secondary/40 hover:text-error"
                    >
        ...
        <div className="px-5 py-3 border-t border-divider flex items-center justify-between shrink-0">
          ...
          <button
            onClick={onClose}
            className="px-4 py-1.5 rounded-lg bg-primary/10 text-text-primary text-sm font-medium hover:bg-primary/20"
          >
            Done
          </button>
        </div>
