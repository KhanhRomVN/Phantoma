...
export function scanRequest(req: NetworkRequest): SecurityIssue[] {
  ...
  const resBodyStr = String(typeof resBody === 'string' ? resBody : JSON.stringify(resBody));
  ...
  for (const [pattern, title, description] of sensitivePatterns) {
    const match = resBodyStr.match(pattern);
    if (match) {
      issues.push({
  ...
  if (/at\s+\w+[\w.]+\s*\(.*:\d+:\d+\)/m.test(resBodyStr) || /stack\s*trace/i.test(resBodyStr))
    issues.push({
      id: 'stack-trace',
  ...
  if (
    /SQL\s+(syntax|error|exception)/i.test(resBodyStr) ||
    /ORA-\d{5}/.test(resBodyStr) ||
    /mysql_fetch/i.test(resBodyStr)
  )
    issues.push({
      id: 'sql-error',
  ...
  if (/http:\/\/(?!localhost|127\.0\.0\.1)/i.test(resBodyStr))
    issues.push({
      id: 'mixed-content',
  ...
  const scriptTags = resBodyStr.match(scriptSrcPattern) || [];
