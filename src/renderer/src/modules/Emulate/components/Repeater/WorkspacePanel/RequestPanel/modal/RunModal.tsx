...
import type { PayloadItem, RunResult } from '../types';
...
export function RunModal({
  ...
  const progressPercent = totalRequests > 0 ? Math.round((currentProgress / totalRequests) * 100) : 0;
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-border shrink-0">
          <div>
            <h2 className="text-sm font-medium text-text-primary">
              {isCompleted ? 'Execution Complete' : isRunning ? 'Sending Requests...' : 'Confirm Execution'}
            </h2>
            <p className="text-xs text-text-secondary mt-0.5">
              {totalRequests} request{totalRequests > 1 ? 's' : ''}
              {hasPayload && ` with ${enabledPayloads.length} payload${enabledPayloads.length > 1 ? 's' : ''}`}
            </p>
          </div>
          <button
            onClick={handleClose}
            ...
            <div className="space-y-4">
              <div className="flex flex-col items-center gap-2 py-4">
                ...
                <p className="text-sm text-text-primary text-center">
                  Ready to send <span className="font-bold text-primary">{totalRequests}</span> request
                  {totalRequests > 1 ? 's' : ''}
                </p>
              </div>
                  ...
                  <div className="border border-border rounded overflow-hidden max-h-64 overflow-y-auto">
                    <table className="w-full text-xs">
                      <thead className="sticky top-0 bg-table-headerBg border-b border-border">
                        <tr>
                          <th className="w-10 px-3 py-1.5 text-left text-text-secondary font-medium">#</th>
                          <th className="px-3 py-1.5 text-left text-text-secondary font-medium">Payload</th>
                          <th className="px-3 py-1.5 text-left text-text-secondary font-medium">Value</th>
                        </tr>
                      </thead>
                      <tbody>
                        {enabledPayloads.flatMap((payload) =>
                          payload.values.map((value, vi) => (
                            ...
                            >
                              ...
                              <td className="px-3 py-1.5 text-text-primary font-medium">{payload.name}</td>
                  ...
                  <table className="w-full text-xs">
                    <thead className="sticky top-0 bg-table-headerBg border-b border-border">
                      <tr>
                        ...
                        <th className="px-3 py-1.5 text-left text-text-secondary font-medium">Payload</th>
                        <th className="px-3 py-1.5 text-left text-text-secondary font-medium">Value</th>
                        <th className="w-16 px-3 py-1.5 text-center text-text-secondary font-medium">Status</th>
                        <th className="w-20 px-3 py-1.5 text-right text-text-secondary font-medium">Duration</th>
                      </tr>
                    </thead>
                    <tbody>
                      {results.map((result, index) => (
                        ...
                        >
                          ...
                          <td className="px-3 py-1.5 text-text-primary font-medium">{result.payloadName}</td>
