      ...
      if (match) {
        ...
        if (selected && varName === selected.payloadName) {
          const tooltipText =
            ...
            varName +
            ' | All values: ' +
            (allValues ? allValues.join(', ') : 'none');
          return (
            <span key={i} className="text-primary font-medium cursor-default" title={tooltipText}>
              {selected.value}
            </span>
          );
        }
        ...
        return (
          <span key={i} className="text-amber-400 cursor-default" title={tooltipText}>
            {part}
          </span>
        );
      }
