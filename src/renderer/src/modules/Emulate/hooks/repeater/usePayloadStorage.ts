...
export function usePayloadStorage(options: UsePayloadStorageOptions = {}) {
  ...
  useEffect(() => {
    if (autoSave && !isLoading) savePayloads(payloads);
  }, [payloads, autoSave, isLoading, savePayloads]);

  return { payloads, isLoading, loadPayloads, savePayloads, addPayload, updatePayload, deletePayload, togglePayload, getPayloadByName, getEnabledPayloads, clearPayloads };
}
...
export function usePayloadValues(payloadName: string, targetId?: string | null) {
  ...
  const loadValues = useCallback(() => {
    ...
    try {
      ...
      if (data) {
        ...
        else if (parsed.values) setValues(parsed.values);
        else setValues([]);
      } else setValues([]);
    } catch { setValues([]); } finally { setIsLoading(false); }
  }, [targetId, payloadName]);

  const saveValues = useCallback((newValues: string[]) => {
    try {
      const key = getPayloadStorageKey(targetId || 'default', payloadName, 'files');
  ...
  return { values, isLoading, loadValues, saveValues };
}
