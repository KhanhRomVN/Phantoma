...
export function HostCard({ host, selected, onClick }: HostCardProps) {
  return (
    <div
      onClick={onClick}
      className={cn(
        'bg-zinc-950 border rounded-md p-2.5 cursor-pointer transition-all',
        selected
    ...
    >
      <div className="flex items-center gap-2 mb-1.5">
        ...
        <span className="text-[10.5px] text-zinc-500">{host.hostname}</span>
        <span className="ml-auto text-[10px] px-1.5 py-0 rounded bg-purple-500/10 text-purple-400 border border-purple-500/20">
          {host.os}
        </span>
      </div>
