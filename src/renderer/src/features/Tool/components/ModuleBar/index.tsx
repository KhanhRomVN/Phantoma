...
const NAV_MODULES: NavModuleConfig[] = [
  ...
  },
  {
    ...
    children: [
      ...
      { id: 'recon-test', title: 'Test', disabled: false },
    ],
  },
  {
    id: 'scanner',
...
function NavLogo() {
  return (
    <div className="w-9 h-9 rounded-lg flex items-center justify-center cursor-pointer shrink-0" title="PHANTOM v2.5.0">
...
function NavIcon({ module }: { module: PhantomModule }) {
  ...
  switch (module) {
    case 'dashboard': return <svg {...p}><rect x="1" y="1" width="6" height="6" rx="1" /><rect x="9" y="1" width="6" height="6" rx="1" /><rect x="1" y="9" width="6" height="6" rx="1" /><rect x="9" y="9" width="6" height="6" rx="1" /></svg>;
    ...
    case 'c2': return <svg {...p}><rect x="2" y="2" width="5" height="5" rx="1" /><rect x="9" y="2" width="5" height="5" rx="1" /><rect x="2" y="9" width="5" height="5" rx="1" /><path d="M11.5 9v3m0 0l-2-1.5m2 1.5l2-1.5" /></svg>;
    case 'settings': return <SettingsIcon className="w-4 h-4" strokeWidth={1.3} />;
    case 'target': return <CrosshairIcon className="w-4 h-4" strokeWidth={1.3} />;
    default: return null;
  }
}
...
function NavButton({ module, title, isActive, activeClass, dotColor, onClick }: {
  module: PhantomModule; title: string; isActive: boolean; activeClass: string; dotColor?: string; onClick: () => void;
}) {
  return (
    <button onClick={onClick} className={cn(
      ...
      <div className="w-5 h-5 flex items-center justify-center shrink-0"><NavIcon module={module} /></div>
      <span className="text-[11px] font-medium truncate flex-1 text-left">{title}</span>
      {dotColor && !isActive && <span className={cn('absolute top-2 right-2 w-1.5 h-1.5 rounded-full', dotColor)} />}
    </button>
  );
}

// ─── SubMenuItemButton with tree line ───────────────────────────────────────
function SubMenuItemButton({ item, onSelect, isActive, isFirst, isLast }: {
  item: SubMenuItem; onSelect: () => void; isActive: boolean; isFirst?: boolean; isLast?: boolean;
}) {
  return (
    <div className="relative pl-5">
      {/* Vertical line - only for non-last items, or connect through all */}
      {!isLast && (
        <div className="absolute left-[13px] top-0 bottom-0 w-px bg-[#2a3548]/40" />
      )}
      
      {/* Horizontal connector line */}
      <div className="absolute left-[13px] top-1/2 w-3 h-px bg-[#2a3548]/40 -translate-y-1/2" />
      
      <button onClick={onSelect} disabled={item.disabled} className={cn(
        'w-full flex items-center gap-2 pl-3 py-1.5 text-left transition-all duration-200 text-sm bg-[#0d1017] relative',
        isActive && !item.disabled ? 'text-white' : item.disabled ? 'text-[#3a4558] cursor-not-allowed opacity-50' : 'text-[#6b7a96] hover:text-[#c5cfe0]',
      )}>
        <span className={cn(
          'w-1.5 h-1.5 rounded-full shrink-0 relative z-10',
          isActive && !item.disabled ? 'bg-white ring-1 ring-white ring-opacity-50' : item.disabled ? 'bg-[#3a4558]' : 'bg-[#1e2535] group-hover:bg-[#6b7a96]'
        )} />
...
export function ModuleBar({ active, onSelect, activeSubItem, onSubItemSelect }: {
  active: PhantomModule; onSelect: (m: PhantomModule) => void; activeSubItem?: string | null; onSubItemSelect?: (subItemId: string) => void;
}) {
  ...
  return (
    <div className="relative">
      <div className="w-[240px] shrink-0 bg-[#0f1319] border-r border-[#1e2535] flex flex-col z-10 overflow-y-auto [&::-webkit-scrollbar]:w-0 h-full">
        <div className="w-full h-[37px] flex items-center px-3 shrink-0"><NavLogo /><span className="ml-2 text-xs font-mono text-cyan-400">PHANTOM v2.5.0</span></div>
        <div className="w-full h-px bg-[#1e2535] shrink-0" />

        <div className="flex flex-col gap-0.5 w-full px-2 py-2">
          {NAV_MODULES.map((item) => (
            <div key={item.id}>
              <NavButton module={item.id} title={item.title} isActive={active === item.id} activeClass={item.activeClass} dotColor={item.dotColor} onClick={() => onSelect(item.id)} />
        ...
        <div className="mt-auto flex flex-col gap-1 px-2 pb-3">
          <NavButton module={'target' as PhantomModule} title="Target Manager" isActive={active === 'target'} activeClass="bg-cyan-500/10 text-cyan-400 border-cyan-500/30" onClick={() => onSelect('target' as PhantomModule)} />
          <NavButton module={'settings' as PhantomModule} title="Settings" isActive={active === 'settings'} activeClass="bg-amber-500/10 text-amber-400 border-amber-500/30" onClick={() => onSelect('settings' as PhantomModule)} />
        </div>
      </div>
    </div>
  );
