...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className="text-[9px] font-bold font-mono uppercase px-1.5 py-0.5 rounded-sm" style={{ color: c.color, border: `1px solid ${c.color}40`, background: `${c.color}12` }}>{c.label}</span>
  );
}
...
function ExpandableRaw({ content }: { content: string }) {
  ...
  return (
    <div className="mt-2">
      ...
      {content.split('\n').length > 4 && (
        <button onClick={() => setExpanded(!expanded)} className="text-[10px] font-mono text-[#0af] hover:text-[#5cf] mt-1">{expanded ? 'Show less' : 'Show more'}</button>
      )}
    </div>
  );
}

export function SQLi({ data }: { data: WebsiteAttackData }) {
  const results = data.sqliResults;
  const successCount = results.filter(r => r.status === 'success' && r.vulnerable).length;
  const totalCredentials = results.reduce((sum, r) => sum + (r.credentials?.length || 0), 0);

  return (
    <div className="flex-1 overflow-y-auto p-3">
      <div className="grid grid-cols-2 gap-2">
        <div className="col-span-2 grid grid-cols-4 gap-2 mb-1">
          <StatBox label="Tests Run" value={results.length} sub="SQLi scans" accent="#0af" />
          <StatBox label="Vulnerable" value={successCount} sub="confirmed SQLi" accent="#30d158" />
          <StatBox label="Databases" value={results.reduce((sum, r) => sum + (r.databaseNames?.length || 0), 0)} sub="extracted" accent="#ff2d55" />
          <StatBox label="Credentials" value={totalCredentials} sub="dumped" accent="#f5a623" />
        </div>
        ...
        {results.length === 0 ? (
          <div className="col-span-2 bg-[#0d1017] border border-[#1c2333] rounded p-6 text-center">
            ...
            <div className="text-[11px] font-mono text-[#c8d6f0]">No SQL injection tests run yet</div>
            <div className="text-[10px] font-mono text-[#2a3548] mt-1">Supports: Error-based, Blind, Time-based, Union, Stacked queries</div>
          </div>
        ) : (
            ...
            <div key={idx} className="bg-[#0d1017] border border-[#1c2333] rounded p-3 col-span-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="bg-[#0a0e14] border border-[#111827] rounded p-2">
                  ...
                  <KV k="Technique" v={result.config.technique.toUpperCase()} />
                  <KV k="DBMS" v={result.config.dbms?.toUpperCase() || 'Auto-detect'} />
                  <KV k="Level" v={result.config.level} />
                  {result.parameter && <KV k="Parameter" v={result.parameter} vc="text-[#f5a623]" />}
                </div>
                <div className="bg-[#0a0e14] border border-[#111827] rounded p-2">
                  ...
                  <KV k="Databases" v={result.databaseNames?.join(', ') || '—'} vc="text-[#f5a623]" />
                  <KV k="Tables" v={result.databases?.reduce((sum, db) => sum + db.tables.length, 0) || 0} />
                  <KV k="Credentials" v={result.credentials?.length || 0} vc={result.credentials?.length ? 'text-[#30d158]' : 'text-[#c8d6f0]'} />
                </div>
              </div>

              {result.credentials && result.credentials.length > 0 && (
                <div className="mt-2 bg-[#0a0e14] border border-[#111827] rounded p-2">
                  <div className="text-[10px] font-mono text-[#30d158] uppercase mb-1">Extracted Credentials</div>
                  {result.credentials.map((cred, ci) => (
                    <div key={ci} className="flex gap-3 py-0.5 border-b border-[#111827] last:border-0">
                      ...
                      <span className="text-[10px] font-mono text-[#2a3548]">{cred.database}.{cred.table}</span>
                    </div>
                  ))}
                </div>
              )}
        ...
        <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3 col-span-2">
          <SectionHeader accent="#ff2d55">Launch SQL Injection</SectionHeader>
          <div className="grid grid-cols-2 gap-2 mb-2">
            <div className="col-span-2">
              <label className="text-[10px] font-mono text-[#c8d6f0] uppercase block mb-1">Target URL</label>
              <input type="text" defaultValue={data.targetUrl + '/search.php?query=test'} className="w-full h-7 bg-[#040608] border border-[#1c2333] rounded px-2 text-[11px] font-mono text-[#0af] outline-none" style={{ caretColor: '#0af' }} />
            </div>
            <div>
              <label className="text-[10px] font-mono text-[#c8d6f0] uppercase block mb-1">Technique</label>
            ...
            <div>
              <label className="text-[10px] font-mono text-[#c8d6f0] uppercase block mb-1">DBMS</label>
              <select className="w-full h-7 bg-[#040608] border border-[#1c2333] rounded px-2 text-[11px] font-mono text-[#c8d6f0] outline-none">
            ...
            <div>
              <label className="text-[10px] font-mono text-[#c8d6f0] uppercase block mb-1">Level (1-5)</label>
              <input type="number" min={1} max={5} defaultValue={3} className="w-full h-7 bg-[#040608] border border-[#1c2333] rounded px-2 text-[11px] font-mono text-[#f5a623] outline-none" />
            </div>
            <div>
              <label className="text-[10px] font-mono text-[#c8d6f0] uppercase block mb-1">Cookie (optional)</label>
              <input type="text" placeholder="PHPSESSID=..." className="w-full h-7 bg-[#040608] border border-[#1c2333] rounded px-2 text-[11px] font-mono text-[#c8d6f0] outline-none placeholder:text-[#2a3548]" />
            </div>
          </div>
          <button className="h-8 px-4 rounded text-[11px] font-bold font-mono transition-colors" style={{ background: '#ff2d5515', border: '1px solid #ff2d5530', color: '#ff2d55' }}>💉 Launch SQLi Scan</button>
        </div>
      </div>
    </div>
  );
