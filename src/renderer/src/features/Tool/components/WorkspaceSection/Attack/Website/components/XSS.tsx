...
export function XSS({ data }: { data: WebsiteAttackData }) {
  const results = data.xssResults;
  const successCount = results.filter(r => r.status === 'success' && r.vulnerable).length;
  const reflected = results.filter(r => r.config.type === 'reflected').length;
  const stored = results.filter(r => r.config.type === 'stored').length;

  return (
    <div className="flex-1 overflow-y-auto p-3">
      <div className="grid grid-cols-2 gap-2">
        <div className="col-span-2 grid grid-cols-4 gap-2 mb-1">
          <StatBox label="Tests Run" value={results.length} sub="XSS scans" accent="#0af" />
          <StatBox label="Vulnerable" value={successCount} sub="confirmed XSS" accent="#30d158" />
          <StatBox label="Reflected" value={reflected} sub="non-persistent" accent="#f5a623" />
          <StatBox label="Stored" value={stored} sub="persistent" accent="#ff2d55" />
        </div>
        ...
        {results.length === 0 ? (
          <div className="col-span-2 bg-[#0d1017] border border-[#1c2333] rounded p-6 text-center">
            ...
            <div className="text-[11px] font-mono text-[#c8d6f0]">No XSS tests run yet</div>
            <div className="text-[10px] font-mono text-[#2a3548] mt-1">Supports: Reflected, Stored, DOM-based XSS</div>
          </div>
        ) : (
            ...
            <div key={idx} className="bg-[#0d1017] border border-[#1c2333] rounded p-3 col-span-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="bg-[#0a0e14] border border-[#111827] rounded p-2">
                  ...
                  <KV k="Type" v={result.config.type.toUpperCase()} />
                  <KV k="Parameter" v={result.parameter || '—'} vc="text-[#f5a623]" />
                </div>
                <div className="bg-[#0a0e14] border border-[#111827] rounded p-2">
                  ...
                  {result.payload && <KV k="Payload" v={result.payload} vc="text-[#ff6b35]" />}
                  {result.proofOfConcept && <KV k="PoC" v={result.proofOfConcept} />}
                </div>
              </div>
        ...
        <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3 col-span-2">
          <SectionHeader accent="#ff9f0a">Launch XSS Scan</SectionHeader>
          <div className="grid grid-cols-2 gap-2 mb-2">
            <div className="col-span-2">
              <label className="text-[10px] font-mono text-[#c8d6f0] uppercase block mb-1">Target URL</label>
              <input type="text" defaultValue={data.targetUrl + '/search.php?q='} className="w-full h-7 bg-[#040608] border border-[#1c2333] rounded px-2 text-[11px] font-mono text-[#0af] outline-none" style={{ caretColor: '#0af' }} />
            </div>
            <div>
              <label className="text-[10px] font-mono text-[#c8d6f0] uppercase block mb-1">XSS Type</label>
            ...
            <div>
              <label className="text-[10px] font-mono text-[#c8d6f0] uppercase block mb-1">Custom Payload</label>
              <input type="text" placeholder="<script>alert(1)</script>" className="w-full h-7 bg-[#040608] border border-[#1c2333] rounded px-2 text-[11px] font-mono text-[#ff6b35] outline-none placeholder:text-[#2a3548]" style={{ caretColor: '#ff6b35' }} />
            </div>
          </div>
          <button className="h-8 px-4 rounded text-[11px] font-bold font-mono transition-colors" style={{ background: '#ff9f0a15', border: '1px solid #ff9f0a30', color: '#ff9f0a' }}>🏷️ Launch XSS Scan</button>
        </div>
      </div>
    </div>
  );
