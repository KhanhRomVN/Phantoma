import type { WebsiteAttackData, DeserializationLanguage } from '../types/website-attack';
...
const LANGUAGE_GADGETS: Record<DeserializationLanguage, string[]> = {
  java: ['CommonsCollections1-7', 'Spring1', 'Groovy1', 'JDK7u21', 'JRE8u20'],
  php: ['__wakeup', '__destruct', '__toString', 'Phar deserialization'],
  dotnet: ['ObjectDataProvider', 'ActivitySurrogateSelector', 'WindowsIdentity'],
  python: ['pickle REDUCE', '__reduce__', 'PyYAML deserialization'],
  ruby: ['Marshal.load', 'YAML.load', 'ERB template injection'],
};

export function Deserialization({ data }: { data: WebsiteAttackData }) {
  const results = data.deserializationResults;
    ...
    <div className="flex-1 overflow-y-auto p-3">
      <div className="grid grid-cols-2 gap-2">
        <div className="col-span-2 grid grid-cols-4 gap-2 mb-1">
          <StatBox label="Tests Run" value={results.length} sub="deserialization" accent="#0af" />
          <StatBox label="Vulnerable" value={successCount} sub="confirmed" accent="#30d158" />
          <StatBox label="RCE Achieved" value={rceCount} sub="shell obtained" accent="#ff2d55" />
          <StatBox label="Languages" value={[...new Set(results.map(r => r.config.language))].length} sub="tested" accent="#f5a623" />
        </div>
        ...
        {results.length === 0 ? (
          <div className="col-span-2 bg-[#0d1017] border border-[#1c2333] rounded p-6 text-center">
            ...
            <div className="text-[11px] font-mono text-[#c8d6f0]">No deserialization tests run yet</div>
            <div className="text-[10px] font-mono text-[#2a3548] mt-1">Supports: Java (ysoserial), PHP, .NET, Python (pickle), Ruby</div>
          </div>
        ) : (
            ...
            <div key={idx} className="bg-[#0d1017] border border-[#1c2333] rounded p-3 col-span-2">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-[#0af12] text-[#0af] border border-[#0af30]">{result.config.language.toUpperCase()}</span>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="bg-[#0a0e14] border border-[#111827] rounded p-2">
                  ...
                  <KV k="Language" v={result.config.language.toUpperCase()} />
                  <KV k="Gadget Chain" v={result.config.gadgetChain || '—'} vc="text-[#f5a623]" />
                </div>
                <div className="bg-[#0a0e14] border border-[#111827] rounded p-2">
                  ...
                  {result.commandOutput && <KV k="Output" v={result.commandOutput} />}
                </div>
              </div>
        ...
        <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3 col-span-2">
          <SectionHeader accent="#ff6b35">Launch Deserialization Attack</SectionHeader>
          <div className="grid grid-cols-2 gap-2 mb-2">
            <div className="col-span-2">
              <label className="text-[10px] font-mono text-[#c8d6f0] uppercase block mb-1">Target URL</label>
              <input type="text" defaultValue={data.targetUrl + '/index.php'} className="w-full h-7 bg-[#040608] border border-[#1c2333] rounded px-2 text-[11px] font-mono text-[#0af] outline-none" style={{ caretColor: '#0af' }} />
            </div>
            <div>
              <label className="text-[10px] font-mono text-[#c8d6f0] uppercase block mb-1">Language</label>
            ...
            <div>
              <label className="text-[10px] font-mono text-[#c8d6f0] uppercase block mb-1">Command (for RCE)</label>
              <input type="text" placeholder="id" className="w-full h-7 bg-[#040608] border border-[#1c2333] rounded px-2 text-[11px] font-mono text-[#c8d6f0] outline-none placeholder:text-[#2a3548]" style={{ caretColor: '#c8d6f0' }} />
            </div>
          </div>
          <button className="h-8 px-4 rounded text-[11px] font-bold font-mono transition-colors" style={{ background: '#ff6b3515', border: '1px solid #ff6b3530', color: '#ff6b35' }}>🧬 Launch Deserialization</button>
        </div>
      </div>
    </div>
  );
