...
interface HistoryEntry {
  ...
  target: string;
  timestamp: number;
  data: WebsiteAttackData;
  summary: string;
}

interface HistoryProps {
  onSelectHistory: (data: WebsiteAttackData) => void;
  onBack: () => void;
}
...
const MOCK_HISTORY: HistoryEntry[] = [
  {
    ...
    data: {} as WebsiteAttackData,
    summary: 'SQLi + XSS + LFI + SSRF + Cmd Injection — 6 vulns, 1 web shell',
  },
  {
    id: '2',
    ...
    data: {} as WebsiteAttackData,
    summary: 'XSS + SQLi found — 2 credentials extracted',
  },
];
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="flex items-center gap-2 px-3 py-2 bg-[#060810] border-b border-[#1c2333] shrink-0">
        ...
        <span className="text-[13px] font-mono text-[#c8d6f0]">Web Attack History</span>
      </div>
          ...
          entries.map((entry) => (
            ...
            >
              <div className="flex items-center justify-between mb-1">
                <span className="text-[14px] font-mono font-bold text-[#0af]">{entry.target}</span>
                <span className="text-[10px] font-mono text-[#c8d6f0]">{getTimeAgo(entry.timestamp)}</span>
              </div>
              <div className="text-[11px] font-mono text-[#c8d6f0]">{entry.summary}</div>
              <div className="text-[10px] font-mono text-[#2a3548] mt-1">{formatDate(entry.timestamp)}</div>
            </div>
          ))
        )}
      </div>
