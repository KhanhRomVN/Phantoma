...
export function XXE({ data }: { data: WebsiteAttackData }) {
  const results = data.xxeResults;
  const successCount = results.filter(r => r.status === 'success' && r.vulnerable).length;
  const fileReadCount = results.filter(r => r.fileContents).length;

  return (
    <div className="flex-1 overflow-y-auto p-3">
      <div className="grid grid-cols-2 gap-2">
        <div className="col-span-2 grid grid-cols-4 gap-2 mb-1">
          <StatBox label="Tests Run" value={results.length} sub="XXE probes" accent="#0af" />
          <StatBox label="Vulnerable" value={successCount} sub="confirmed XXE" accent="#30d158" />
          <StatBox label="Files Read" value={fileReadCount} sub="via XXE" accent="#ff2d55" />
          <StatBox label="SSRF via XXE" value={results.filter(r => r.ssrfAchieved).length} sub="achieved" accent="#f5a623" />
        </div>
        ...
        {results.length === 0 ? (
          <div className="col-span-2 bg-[#0d1017] border border-[#1c2333] rounded p-6 text-center">
            ...
            <div className="text-[11px] font-mono text-[#c8d6f0]">No XXE tests run yet</div>
            <div className="text-[10px] font-mono text-[#2a3548] mt-1">Supports: File Read, SSRF, DoS (Billion Laughs), OOB XXE</div>
          </div>
        ) : (
            ...
            <div key={idx} className="bg-[#0d1017] border border-[#1c2333] rounded p-3 col-span-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="bg-[#0a0e14] border border-[#111827] rounded p-2">
                  ...
                  <KV k="File Target" v={result.config.fileToRead || '—'} vc="text-[#ff6b35]" />
                </div>
                <div className="bg-[#0a0e14] border border-[#111827] rounded p-2">
                  ...
                  <KV k="SSRF via XXE" v={result.ssrfAchieved ? 'YES ✓' : 'NO ✗'} vc={result.ssrfAchieved ? 'text-[#f5a623]' : 'text-[#c8d6f0]'} />
                  <KV k="DoS Achieved" v={result.dosAchieved ? 'YES ⚠' : 'NO'} vc={result.dosAchieved ? 'text-[#ff2d55]' : 'text-[#c8d6f0]'} />
                </div>
              </div>

              {result.fileContents && (
                <div className="mt-2 bg-[#0a0e14] border border-[#111827] rounded p-2">
                  <div className="text-[10px] font-mono text-[#30d158] uppercase mb-1">File Contents (via XXE)</div>
        ...
        <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3 col-span-2">
          <SectionHeader accent="#5e5ce6">Launch XXE Attack</SectionHeader>
          <div className="grid grid-cols-2 gap-2 mb-2">
            <div className="col-span-2">
              <label className="text-[10px] font-mono text-[#c8d6f0] uppercase block mb-1">Target URL (XML endpoint)</label>
              <input type="text" defaultValue={data.targetUrl + '/upload.php'} className="w-full h-7 bg-[#040608] border border-[#1c2333] rounded px-2 text-[11px] font-mono text-[#0af] outline-none" style={{ caretColor: '#0af' }} />
            </div>
            <div>
              <label className="text-[10px] font-mono text-[#c8d6f0] uppercase block mb-1">Target File to Read</label>
              <input type="text" placeholder="file:///etc/passwd" className="w-full h-7 bg-[#040608] border border-[#1c2333] rounded px-2 text-[11px] font-mono text-[#ff6b35] outline-none placeholder:text-[#2a3548]" style={{ caretColor: '#ff6b35' }} />
            </div>
            <div>
              <label className="text-[10px] font-mono text-[#c8d6f0] uppercase block mb-1">OOB DTD Host (optional)</label>
              <input type="text" placeholder="attacker.com" className="w-full h-7 bg-[#040608] border border-[#1c2333] rounded px-2 text-[11px] font-mono text-[#c8d6f0] outline-none placeholder:text-[#2a3548]" style={{ caretColor: '#c8d6f0' }} />
            </div>
          </div>
          <button className="h-8 px-4 rounded text-[11px] font-bold font-mono transition-colors" style={{ background: '#5e5ce615', border: '1px solid #5e5ce630', color: '#5e5ce6' }}>📄 Launch XXE Probe</button>
        </div>
      </div>
    </div>
  );
