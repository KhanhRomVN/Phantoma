...
import { FileCode, Download, Search, Cpu, X, Code, Terminal } from 'lucide-react';
...
function WasmCard({ item, request, onDownload }: { item: WasmItem; request?: NetworkRequest; onDownload?: (item: WasmItem, request?: NetworkRequest) => void }) {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 pt-4 pb-3 border-b border-divider shrink-0 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-text-secondary mt-0.5">WebAssembly modules detected in network traffic</p>
        </div>
        {onClose && (
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2">
        {filtered.map(item => (
          <WasmCard key={item.id} item={item} request={requests.find(r => r.id === item.id)} onDownload={handleDownload} />
        ))}
        ...
        {wasmItems.length === 0 && (
          <div className="flex flex-col items-center justify-center flex-1 py-20 gap-3">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-text-secondary mt-0.5">Navigate to a page that loads WebAssembly</p>
            </div>
          </div>
        )}
      </div>
