            ...
            <table className="w-full text-[11px] font-mono">
              <thead>
                <tr className="border-b border-[#1c2333] bg-[#0a0e14]">
                  <th className="text-left p-1 text-[#c8d6f0] font-normal">Path</th>
                  <th className="text-left p-1 text-[#c8d6f0] font-normal">Method</th>
                  <th className="text-left p-1 text-[#c8d6f0] font-normal">Description</th>
                </tr>
              </thead>
              <tbody>
                {appStructure.endpointMapping.map((ep, idx) => (
                  <tr key={idx} className="border-b border-[#111827]">
                    ...
                    <td className="p-1 text-[#c8d6f0]">{ep.description || '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3 col-span-2">
            ...
            <div className="flex flex-wrap gap-2">
              {appStructure.hiddenPaths.map((path, idx) => (
                <span key={idx} className="text-[11px] font-mono px-2 py-0.5 rounded bg-[#0a0e14] border border-[#1c2333] text-[#ff6b35]">
                  {path}
                </span>
              ))}
            </div>
