      ...
      <div className="space-y-2">
            ...
            <div className="space-y-1">
              {registeredServices.map((svc, i) => (
                <div key={i} className="flex items-center gap-3 py-1 border-b border-[#111827] last:border-0">
                  ...
                  />
                  <span className="text-[12px] font-mono text-[#c8d6f0] flex-1">{svc.service}</span>
                  {svc.url && (
                    <span className="text-[11px] font-mono text-[#0af] truncate max-w-[160px]">{svc.url}</span>
                  )}
                  <span
        ...
        {noiseResults.length > 0 && (
          <div className="bg-[#0d1017] border border-[#f5a62330] rounded p-3">
            ...
            <p className="text-[11px] font-mono text-[#c8d6f0] mb-2 leading-relaxed">
              ...
              {noiseResults.map((result, i) => (
                <div key={i} className="p-2 bg-[#0a0e14] rounded border border-[#1c2333]">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[12px] font-mono font-bold text-[#f5a623]">{result.source}</span>
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-mono text-[#c8d6f0] px-1.5 py-0.5 bg-[#1c2333] rounded">{result.type}</span>
                      <ConfidencePill value={result.confidence} />
                    </div>
                  </div>
                  <div className="text-[11px] font-mono text-[#0af] mb-1 break-all">{result.url}</div>
                  <div className="text-[11px] font-mono text-[#c8d6f0] italic">{result.note}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {noiseResults.length === 0 && registeredServices.length === 0 && (
          <div className="flex items-center justify-center py-12 flex-col gap-2">
            ...
            <span className="text-[12px] font-mono text-[#c8d6f0]">No additional data available</span>
          </div>
        )}
      </div>
    </div>
