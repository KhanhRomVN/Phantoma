...
export function ContactInfo({ data }: { data: PersonData }) {
    ...
    <div className="flex-1 overflow-y-auto p-3">
      <div className="grid grid-cols-2 gap-2">
        <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3 col-span-2">
          <SectionHeader accent="#0af">Contact Information</SectionHeader>
          {contactInfo.email && contactInfo.email.length > 0 && (
            <KV k="Email" v={contactInfo.email.map(e => e).join(', ')} vc="text-[#0af]" />
          )}
