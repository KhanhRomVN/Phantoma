...
function KV({ k, v, vc = 'text-[#c8d6f0]' }: { k: string; v: string | number | React.ReactNode; vc?: string }) {
  return (
    <div className="flex justify-between items-start gap-2 py-1 border-b border-[#111827] last:border-0">
      <span className="text-[11px] font-mono text-[#c8d6f0] uppercase tracking-wide shrink-0">{k}</span>
      <span className={cn('text-[12px] font-mono text-right break-all', vc)}>{v}</span>
    </div>
  );
}
...
function SeverityBadge({ level }: { level?: string }) {
  ...
  return (
    <span
      className="text-[10px] font-mono font-bold px-1.5 py-0.5 rounded tracking-widest"
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        {breachDbs.length > 0 && (
          <div className="col-span-2 bg-[#0d1017] border border-[#1c2333] rounded p-3">
            ...
            <div className="flex flex-wrap gap-1.5">
              {breachDbs.map((db, i) => (
                <span key={i} className="text-[11px] font-mono px-2 py-0.5 rounded" style={{ color: '#ff2d55', backgroundColor: '#ff2d5515', border: '1px solid #ff2d5530' }}>
                  {db}
                </span>
              ))}
            </div>
              ...
              {passwordLeaks.map((leak, idx) => (
                <div key={idx} className="p-2 bg-[#0a0e14] rounded border border-[#1c2333]">
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-[12px] font-mono font-bold text-[#ff2d55]">{leak.source}</span>
                    <div className="flex items-center gap-1.5">
                      {leak.hashType && (
                        <span className="text-[10px] font-mono px-1.5 py-0.5 rounded" style={{ color: '#6a7a9a', backgroundColor: '#1c233340' }}>
                          {leak.hashType}
                        </span>
                      )}
                  ...
                  {leak.detail && (
                    <div className="mt-1.5 pt-1.5 border-t border-[#1c2333]">
                      <p className="text-[11px] font-mono text-[#c8d6f0] leading-relaxed">{leak.detail}</p>
                    </div>
                  )}
                </div>
              ))}
          ...
          <div className="col-span-2 bg-[#0d1017] border border-[#1c2333] rounded p-3">
            ...
            <div className="space-y-2">
              {credentialLeaks.map((leak, idx) => (
                <div key={idx} className="p-2 bg-[#0a0e14] rounded border border-[#1c2333]">
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-[12px] font-mono font-bold text-[#ff6b35]">{leak.source}</span>
                    <SeverityBadge level={leak.severity} />
                  </div>
                  <KV k="Date" v={leak.date} />
            ...
            <div className="space-y-2">
              {pastebinLeaks.map((leak, idx) => (
                <div key={idx} className="p-2 bg-[#0a0e14] rounded border border-[#1c2333]">
                  ...
                  {leak.preview && (
                    <div className="mt-1.5 pt-1.5 border-t border-[#1c2333]">
                      <p className="text-[11px] font-mono text-[#c8d6f0] italic leading-relaxed break-all">{leak.preview}</p>
                    </div>
                  )}
                </div>
              ))}
              ...
              {darkwebMentions.map((mention, idx) => (
                <div key={idx} className="p-2 bg-[#0a0e14] rounded border border-[#1c2333]">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[12px] font-mono font-bold text-[#af52de]">{mention.forum}</span>
                    <span className="text-[11px] font-mono text-[#c8d6f0]">{mention.date}</span>
                  </div>
                  <p className="text-[11px] font-mono text-[#c8d6f0] leading-relaxed">{mention.context}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {totalLeaks === 0 && pastebinLeaks.length === 0 && darkwebMentions.length === 0 && (
          <div className="col-span-2 flex items-center justify-center py-8 flex-col gap-2">
            ...
            <span className="text-[12px] font-mono text-[#c8d6f0]">No leak exposure found</span>
          </div>
        )}
      </div>
    </div>
