            ...
            <table className="w-full text-[11px] font-mono">
              <thead>
                <tr className="border-b border-[#1c2333] bg-[#0a0e14]">
                  <th className="text-left p-2 text-[#c8d6f0] font-normal tracking-wider text-[10px] uppercase">Port</th>
                  <th className="text-left p-2 text-[#c8d6f0] font-normal tracking-wider text-[10px] uppercase">Protocol</th>
                  <th className="text-left p-2 text-[#c8d6f0] font-normal tracking-wider text-[10px] uppercase">State</th>
                  <th className="text-left p-2 text-[#c8d6f0] font-normal tracking-wider text-[10px] uppercase">Service</th>
                  <th className="text-left p-2 text-[#c8d6f0] font-normal tracking-wider text-[10px] uppercase">Banner</th>
                  <th className="text-left p-2 text-[#c8d6f0] font-normal tracking-wider text-[10px] uppercase">Version</th>
                </tr>
              </thead>
              <tbody>
                {data.ports.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="p-4 text-center text-[11px] font-mono text-[#c8d6f0]">
                      No ports scanned
                    </td>
                  </tr>
                ) : (
                  data.ports.map((port, idx) => (
                    <tr key={idx} className="border-b border-[#111827] hover:bg-[#111827] transition-colors">
                      <td className="p-2 font-mono text-[12px] text-[#0af]">{port.port}</td>
                      <td className="p-2 text-[11px] text-[#c8d6f0]">{port.protocol}</td>
                      <td className="p-2">
                        <span className={port.state === 'open' ? 'text-[#30d158]' : port.state === 'filtered' ? 'text-[#f5a623]' : 'text-[#c8d6f0]'}>
                          {port.state}
                        </span>
                      </td>
                      <td className="p-2 text-[11px] text-[#c8d6f0]">{port.service}</td>
                      <td className="p-2 text-[11px] text-[#c8d6f0]">{port.banner || '—'}</td>
                      <td className="p-2 text-[11px] text-[#c8d6f0]">{port.version || '—'}</td>
                    </tr>
                  ))
                )}
              </tbody>
