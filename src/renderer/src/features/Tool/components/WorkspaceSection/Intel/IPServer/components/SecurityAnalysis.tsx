    ...
    <div className="flex-1 overflow-y-auto p-3">
      <div className="grid grid-cols-2 gap-2">
        ...
        <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3 col-span-2">
          ...
          <KV k="OS" v={osDetection.operatingSystem} vc="text-[#c8d6f0]" />
          ...
          <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3 col-span-2">
            ...
            <div className="flex flex-wrap gap-2">
              {exposureList.map((exp, idx) => (
                <span key={idx} className="text-[11px] font-mono px-2 py-1 rounded" style={{ background: '#ff2d5515', border: '1px solid #ff2d5530', color: '#ff2d55' }}>
                  {exp}
                </span>
              ))}
            </div>
            ...
            <table className="w-full text-[11px] font-mono">
              <thead>
                <tr className="border-b border-[#1c2333] bg-[#0a0e14]">
                  <th className="text-left p-2 text-[#c8d6f0] font-normal tracking-wider text-[10px] uppercase">Finding</th>
                  <th className="text-left p-2 text-[#c8d6f0] font-normal tracking-wider text-[10px] uppercase">Severity</th>
                  <th className="text-left p-2 text-[#c8d6f0] font-normal tracking-wider text-[10px] uppercase">CVSS</th>
                  <th className="text-left p-2 text-[#c8d6f0] font-normal tracking-wider text-[10px] uppercase">CVE</th>
                 </tr>
              </thead>
              <tbody>
                {securityFindings.length === 0 ? (
                  ...
                  securityFindings.map((finding, idx) => (
                    <tr key={idx} className="border-b border-[#111827] hover:bg-[#111827] transition-colors">
                      <td className="p-2 font-mono text-[12px] text-[#ff6b35]">{finding.name}</td>
                      ...
                      <td className="p-2 text-[11px] text-[#0af]">{finding.cve || '—'}</td>
                    </tr>
                  ))
                )}
              </tbody>
