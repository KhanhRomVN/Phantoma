...
export default function DomainRecon({ initialDomain = 'phantoma.com' }: DomainReconProps) {
  ...
  const handleMatchesFound = useCallback((matches: any[], total: number) => {
        ...
        <div className="w-[293px] bg-[#0f1319] border-r border-[#1c2333] flex flex-col shrink-0 overflow-hidden">
          ...
          <div className="overflow-y-auto p-1.5 space-y-1 shrink-0" style={{ maxHeight: '35%' }}>
            {sessions.map((sess) => {
              ...
              const riskColor = riskScore !== undefined
                ? riskScore >= 75 ? '#ff2d55' : riskScore >= 50 ? '#f5a623' : '#30d158'
                : undefined;
              return (
                <div
                  key={sess.id}
                ...
                >
                  {isActive && (
                    <div className="absolute left-0 top-1.5 bottom-1.5 w-[2px] rounded-full" style={{ background: statusMeta.color }} />
                  )}
                  <div className="flex items-center gap-2">
                    <div className={cn('w-1.5 h-1.5 rounded-full shrink-0', statusMeta.pulse && 'animate-pulse')} style={{ background: statusMeta.color }} title={statusMeta.label} />
                    <span className="text-[12px] font-mono font-semibold truncate flex-1 min-w-0 leading-tight" style={{ color: isActive ? '#e4e4e7' : '#a1a1aa' }}>
                      {sess.domain}
                    </span>
                    {riskScore !== undefined && (
                      <span className="text-[10px] font-mono font-bold shrink-0 tabular-nums" style={{ color: riskColor }}>
                        {riskScore}
                      </span>
                    )}
                  </div>
                  {sess.ip && (
                    <div className="mt-1 ml-[14px]">
                      <span className="text-[10px] font-mono text-[#3a4558] truncate block">{sess.ip}</span>
                    </div>
                  )}
                  {riskScore !== undefined && (
                  ...
                  {sess.status !== 'done' && (
                    <div className="mt-1 ml-[14px]">
                      <span className="text-[9px] font-mono" style={{ color: statusMeta.color }}>
                        {sess.status === 'queued' ? 'Queued...' : sess.status === 'scanning' ? 'Scanning...' : sess.status === 'error' ? 'Error' : 'Idle'}
                      </span>
                    </div>
                  )}
                </div>
          ...
          {contextMenu && (
            <div ref={menuRef} className="fixed z-50 bg-[#0d1017] border border-[#1c2333] rounded shadow-lg py-1 min-w-[140px]" style={{ top: contextMenu.y, left: contextMenu.x }}>
              <button onClick={() => setContextMenu(null)} className="w-full text-left px-3 py-1.5 text-[12px] font-mono text-[#0af] hover:bg-[#1c2333] transition-colors">
                ▶ Run (Full Scan)
              </button>
              <div className="border-t border-[#1c2333] my-1" />
              <button onClick={() => { removeDomain(contextMenu.sessionId); setContextMenu(null); }} className="w-full text-left px-3 py-1.5 text-[12px] font-mono text-[#ff2d55] hover:bg-[#1c2333] transition-colors">
                ✕ Delete
              </button>
            </div>
          )}
          ...
          <div className="flex flex-col flex-1 overflow-hidden">
            <div className="flex items-center justify-between px-3 h-10 bg-[#0f1319] border-b border-[#1c2333] shrink-0">
              <div className="relative tabs-dropdown-container">
                <button onClick={() => setShowTabsDropdown(!showTabsDropdown)} className="flex items-center gap-2 text-[#c8d6f0] hover:text-[#0af] transition-colors group" title="Switch tab">
                  {showLog ? (
                    <>
                    ...
                    <>
                      {(() => { const iconName = activeGroup?.icon || 'LayoutDashboard'; const TabIcon = getTabIcon(iconName); return <TabIcon className="w-4 h-4 text-[#6a7a9a] group-hover:text-[#0af] transition-colors" />; })()}
                      <span className="text-[13px] font-mono font-bold">{activeGroup?.label || 'Overview'}</span>
                    </>
                  )}
                ...
                {showTabsDropdown && (
                  <div className="absolute left-0 top-8 z-50 w-52 bg-[#0d1017] border border-[#1c2333] rounded shadow-lg py-1">
                    <button onClick={() => { setShowLog(true); setShowTabsDropdown(false); }} className={cn('w-full flex items-center justify-between px-3 py-1.5 text-[12px] font-mono transition-colors', showLog ? 'bg-[#0af15] text-[#0af]' : 'text-[#c8d6f0] hover:bg-[#1c2333]')}>
                      <div className="flex items-center gap-2"><FileClock className="w-3 h-3 shrink-0 -mt-0.5" /><span>Logs</span></div>
                    </button>
                    <div className="border-t border-[#1c2333] my-1" />
                    {categoryGroups.filter((g) => g.isActive).map((group) => (
                      <button key={group.id} onClick={() => { setActiveTab(group.id); setShowLog(false); setShowTabsDropdown(false); }} className={cn('w-full flex items-center justify-between px-3 py-1.5 text-[12px] font-mono transition-colors', !showLog && activeTab === group.id ? 'bg-[#0af15] text-[#0af]' : 'text-[#c8d6f0] hover:bg-[#1c2333]')}>
                        <div className="flex items-center gap-2">
                          {(() => { const ItemIcon = getTabIcon(group.icon); return <ItemIcon className="w-3 h-3 shrink-0 -mt-0.5" style={{ color: group.accent }} />; })()}
                          <span>{group.label}</span>
                        </div>
                        {group.count > 0 && <span className="text-[10px] text-[#6a7a9a]">({group.count})</span>}
                      </button>
                    ))}
                  </div>
                )}
              ...
              <div className="flex items-center gap-2">
                <div className="flex items-center bg-[#040608] border border-[#1c2333] rounded focus-within:border-[#0af30] transition-colors overflow-hidden">
                  <div className="relative">
                    ...
                    <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search data (regex supported)..." spellCheck={false} className="w-[280px] h-7 bg-transparent pl-6 pr-2 text-[11px] font-mono text-[#c8d6f0] outline-none placeholder:text-[#6a7a9a]" style={{ caretColor: '#0af' }} />
                    {searchQuery && (<button onClick={handleClearSearch} className="absolute right-1.5 top-1/2 -translate-y-1/2 text-[#3a4558] hover:text-[#ff2d55] transition-colors"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 6L6 18M6 6l12 12" /></svg></button>)}
                  </div>
                  <div className="w-px h-5 bg-[#1c2333]" />
                  <div className="flex items-center gap-0.5 px-1">
                    <button onClick={() => setMatchCase(!matchCase)} className={cn('h-6 w-6 flex items-center justify-center rounded text-[10px] font-mono transition-colors', matchCase ? 'bg-[#0af15] text-[#0af]' : 'text-[#6a7a9a] hover:text-[#c8d6f0] hover:bg-[#1c2333]')} title="Match Case (Aa)">Aa</button>
                    <button onClick={() => setMatchWholeWord(!matchWholeWord)} className={cn('h-6 w-6 flex items-center justify-center rounded text-[10px] font-mono transition-colors', matchWholeWord ? 'bg-[#0af15] text-[#0af]' : 'text-[#6a7a9a] hover:text-[#c8d6f0] hover:bg-[#1c2333]')} title="Match Whole Word (Ab|)">Ab|</button>
                    <button onClick={() => setUseRegex(!useRegex)} className={cn('h-6 w-6 flex items-center justify-center rounded text-[10px] font-mono transition-colors', useRegex ? 'bg-[#0af15] text-[#0af]' : 'text-[#6a7a9a] hover:text-[#c8d6f0] hover:bg-[#1c2333]')} title="Use Regular Expression (.*)">.*</button>
                  </div>
                </div>
                {searchQuery && (
                  <div className="flex items-center gap-1">
                    <span className="text-[11px] font-mono text-[#6a7a9a] min-w-[80px] text-right">{totalMatches > 0 ? `${currentMatchIndex + 1} of ${totalMatches}` : 'No results'}</span>
                    <button onClick={handlePrevMatch} disabled={totalMatches === 0} className={cn('h-6 w-6 flex items-center justify-center rounded transition-colors', totalMatches === 0 ? 'opacity-30 cursor-not-allowed' : 'bg-[#1c2333] border border-[#2a3548] text-[#6a7a9a] hover:text-[#c8d6f0]')} title="Previous match"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 15l-6-6-6 6" /></svg></button>
                    <button onClick={handleNextMatch} disabled={totalMatches === 0} className={cn('h-6 w-6 flex items-center justify-center rounded transition-colors', totalMatches === 0 ? 'opacity-30 cursor-not-allowed' : 'bg-[#1c2333] border border-[#2a3548] text-[#6a7a9a] hover:text-[#c8d6f0]')} title="Next match"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 9l6 6 6-6" /></svg></button>
                  </div>
                )}
                <button onClick={() => setShowLog(!showLog)} className={cn('h-7 w-7 flex items-center justify-center rounded transition-colors', showLog ? 'bg-[#0af15] border border-[#0af30] text-[#0af]' : 'bg-[#1c2333] border border-[#2a3548] text-[#6a7a9a] hover:text-[#c8d6f0] hover:border-[#0af30]')} title="Scan Log"><FileClock className="w-4 h-4" /></button>
              </div>
            </div>
