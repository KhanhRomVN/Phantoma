...
export function DataPointRow({ dataPoint, compact = false }: DataPointRowProps) {
  const { label, displayValue, value, confidence, source, category, isNoise, verificationStatus } = dataPoint;
  ...
  if (compact) {
    return (
      ...
      >
        ...
        <span className="font-mono flex-1 truncate" style={{ color: accentColor }}>
          {displayValue || String(value).substring(0, 80)}
        </span>
        <ConfidenceBadge value={confidence} />
        <span className="text-[9px] font-mono text-[#3a4558] w-20 truncate text-right" title={source.name}>
          {source.name}
        </span>
      </div>
    );
    ...
    >
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-mono font-bold uppercase tracking-wide" style={{ color: accentColor }}>
            {label}
          </span>
          {isNoise && (
        ...
        <div className="flex items-center gap-2">
          {verificationStatus !== 'unverified' && (
            <span
              ...
              style={{
                color: verificationStatus === 'verified' ? '#30d158' : verificationStatus === 'disputed' ? '#ff2d55' : '#f5a623',
                backgroundColor: verificationStatus === 'verified' ? '#30d15815' : verificationStatus === 'disputed' ? '#ff2d5515' : '#f5a62315',
              }}
            >
      ...
      <div className="text-[12px] font-mono break-all leading-relaxed mb-1.5" style={{ color: accentColor }}>
      ...
      {dataPoint.tags && dataPoint.tags.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-1.5 pt-1.5 border-t border-[#111827]">
          {dataPoint.tags.map((tag, i) => (
            <span key={i} className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-[#1c2333] text-[#6a7a9a]">
              {tag}
            </span>
          ))}
        </div>
