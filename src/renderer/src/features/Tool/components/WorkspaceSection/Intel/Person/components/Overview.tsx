...
export function Overview({ result, onSelectEntity }: OverviewProps) {
  ...
  const identityCount = result.allDataPoints.filter((dp) =>
    dp.category.startsWith('identity_'),
  ).length;
  const contactCount = result.allDataPoints.filter((dp) =>
    dp.category.startsWith('contact_'),
  ).length;
  const socialCount = result.allDataPoints.filter((dp) =>
    dp.category.startsWith('social_'),
  ).length;
  const leakCount = result.allDataPoints.filter((dp) =>
    dp.category.startsWith('leak_'),
  ).length;
  ...
  return (
    <div className="flex-1 overflow-y-auto p-3">
      ...
      <div className="grid grid-cols-4 gap-2 mb-3">
        ...
        <StatBox label="Identity" value={identityCount} sub="fields" accent="#0a84ff" />
        <StatBox label="Social" value={socialCount} sub="profiles" accent="#ff6b35" />
        <StatBox label="Leaks" value={leakCount} sub="exposures" accent={leakCount > 0 ? '#ff2d55' : '#30d158'} />
      </div>
      ...
      <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3 mb-3">
        ...
        <div className="flex justify-between items-center py-1 border-b border-[#111827]">
          <span className="text-[11px] font-mono text-[#6a7a9a] uppercase tracking-wide">Email</span>
          <span className="text-[12px] font-mono text-[#0af]">{result.query.value}</span>
        </div>
        {result.targetName && (
          <div className="flex justify-between items-center py-1 border-b border-[#111827]">
            <span className="text-[11px] font-mono text-[#6a7a9a] uppercase tracking-wide">Name</span>
            <span className="text-[12px] font-mono text-[#c8d6f0]">{result.targetName}</span>
          </div>
        )}
        <div className="flex justify-between items-center py-1 border-b border-[#111827]">
