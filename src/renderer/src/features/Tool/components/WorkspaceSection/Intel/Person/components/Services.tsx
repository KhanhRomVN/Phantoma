...
export function Services({ dataPoints, activeGroup }: ServicesProps) {
  const confirmed = dataPoints.filter((dp) =>
    dp.category === 'service_registered' || dp.tags?.includes('confirmed'),
  ).length;
  const unconfirmed = dataPoints.filter((dp) =>
    dp.category === 'service_unconfirmed' || dp.tags?.includes('false-positive'),
  ).length;
