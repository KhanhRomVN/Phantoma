  ...
  return (
    <div className="flex items-center justify-between py-1.5 border-b border-[#111827] last:border-0">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[11px] font-mono text-[#c8d6f0] uppercase tracking-wide">{meta.label}</span>
      </div>
      <span className="text-[12px] font-mono" style={{ color: meta.color }}>{handle}</span>
    </div>
  );
}
    ...
    <div className="flex-1 overflow-y-auto p-3">
      <div className="grid grid-cols-2 gap-2">
            ...
            {Object.entries(socialMedia).map(([platform, handle]) => {
              ...
              return (
                <div key={platform} className="flex items-center gap-2">
                  <span className="text-[11px] font-mono text-[#c8d6f0] w-24 shrink-0">{meta.label}</span>
                  ...
                  <span className="text-[10px] font-mono w-10 text-right" style={{ color: found ? meta.color : '#2a3548' }}>
                    {found ? 'FOUND' : 'N/A'}
                  </span>
                </div>
              );
        ...
        <div className="col-span-2 bg-[#0d1017] border border-[#1c2333] rounded p-3">
          ...
          {activeProfiles === 0 && (
            <span className="text-[11px] font-mono text-[#c8d6f0]">No social media profiles found</span>
          )}
        </div>
      </div>
    </div>
