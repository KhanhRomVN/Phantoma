              ...
              <table className="w-full text-[11px] font-mono">
                <thead>
                  <tr className="border-b border-[#1c2333] bg-[#0a0e14]">
                    <th className="text-left p-2 text-[#c8d6f0] font-normal">Package</th>
                    <th className="text-left p-2 text-[#c8d6f0] font-normal">Version</th>
                    <th className="text-left p-2 text-[#c8d6f0] font-normal">Vulnerable</th>
                    <th className="text-left p-2 text-[#c8d6f0] font-normal">CVE</th>
                  </tr>
                </thead>
                <tbody>
                  {packageDeps.map((dep, idx) => (
                    <tr key={idx} className="border-b border-[#111827] hover:bg-[#111827]">
                      ...
                      <td className="p-2 text-[#c8d6f0]">{dep.version}</td>
                      <td className="p-2">
            ...
            {vulnerableDeps.map((dep, idx) => (
              <div key={idx} className="mb-2 p-2 bg-[#0a0e14] rounded">
                <div className="flex justify-between">
                  <span className="text-[12px] font-mono text-[#0af]">{dep.name}@{dep.version}</span>
                  <span className="text-[10px] text-[#ff2d55]">CRITICAL</span>
                </div>
                <div className="text-[10px] text-[#c8d6f0] mt-1">CVE: {dep.cve?.join(', ')}</div>
              </div>
            ))}
          </div>
        )}
