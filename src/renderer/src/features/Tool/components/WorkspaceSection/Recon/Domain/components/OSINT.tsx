      ...
      <div className="grid grid-cols-2 gap-2">
          ...
          {data.breaches.map((breach, i) => (
            <div key={i} className="mb-2 pb-2 border-b border-[#111827] last:border-0">
              <div className="flex justify-between items-center">
                <span className="text-[11px] font-mono text-[#ff6b35]">{breach.name}</span>
                <span className="text-[9px] font-mono text-[#3a4558]">{breach.date}</span>
              </div>
              <div className="text-[10px] font-mono text-[#8da0c0] mt-0.5">
        ...
        <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3">
          ...
          <div className="flex flex-wrap gap-1">
            {data.harvestedEmails.map((email, i) => (
              <span key={i} className="text-[10px] font-mono text-[#0af] bg-[#0af10] px-2 py-0.5 rounded border border-[#0af30]">
                {email}
              </span>
            ))}
          </div>
        ...
        <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3">
          ...
          {data.googleDorks.map((dork, i) => (
            <div key={i} className="mb-1 pb-1 border-b border-[#111827] last:border-0">
              <div className="text-[9px] font-mono text-[#3a4558] truncate">{dork.query}</div>
              <div className="text-[8px] font-mono text-[#2a3548]">{dork.resultCount} results</div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3">
          ...
          {data.waybackSnapshots.map((snapshot, i) => (
            <div key={i} className="mb-1 pb-1 border-b border-[#111827] last:border-0">
              <div className="text-[9px] font-mono text-[#8da0c0] truncate">{snapshot.url}</div>
              <div className="text-[8px] font-mono text-[#2a3548]">{new Date(snapshot.timestamp).toLocaleDateString()}</div>
            </div>
          ))}
        </div>
            ...
            <div key={i} className="mb-2 pb-2 border-b border-[#111827] last:border-0">
              ...
              <div className="flex flex-wrap gap-1 mt-1">
                {cert.domains.slice(0, 4).map((domain, idx) => (
                  <span key={idx} className="text-[8px] font-mono text-[#6a7a9a] bg-[#0a0e14] px-1 py-0.5 rounded">
                    {domain}
                  </span>
                ))}
                {cert.domains.length > 4 && (
                  <span className="text-[8px] text-[#2a3548]">+{cert.domains.length - 4}</span>
                )}
              </div>
            </div>
          ))}
