...
import { PortService } from './components/PortService';
import { OSDetection } from './components/OSDetection';
import { SecurityAnalysis } from './components/SecurityAnalysis';
import { InfrastructureExposure } from './components/InfrastructureExposure';
import { IPServerOverview as Overview } from './components/Overview';
import { IPServerLog as Log } from './components/Log';
import { Search } from './components/Search';
import { History } from './components/History';
import type { IPServerData } from './types/ip-server-data';
import ipServerData from './data/phantoma-server.json';
import { Activity, Globe, Server, Cpu, AlertTriangle, Shield, Terminal } from 'lucide-react';
...
const DEFAULT_SESSIONS: IPSession[] = [
  {
    ...
    progress: 100,
    riskScore: 58,
    stats: { openPorts: 3, vulnerabilities: 2, exposures: 1 },
  },
];

async function fetchIPData(ip: string): Promise<IPServerData | null> {
  console.log(`[IPServerRecon] Fetching data for: ${ip}`);
  const sampleData = ipServerData as IPServerData;
...
const SUB_TABS = [
  ...
  { id: 'ports', label: 'Ports & Services', accent: '#30d158' },
  { id: 'os', label: 'OS Detection', accent: '#0a84ff' },
  { id: 'security', label: 'Security', accent: '#64d2ff' },
  { id: 'infrastructure', label: 'Infrastructure', accent: '#32d74b' },
  { id: 'log', label: 'Log', accent: '#5e5ce6' },
] as const;

type SubTabId = (typeof SUB_TABS)[number]['id'];

interface IPServerReconProps {
  initialIP?: string;
}

export default function IPServerRecon({ initialIP = '104.18.32.11' }: IPServerReconProps) {
  ...
  const [historyViewData, setHistoryViewData] = useState<IPServerData | null>(null);
  const [data, setData] = useState<IPServerData | null>(null);
  ...
  const addIP = useCallback(() => {
    ...
    const sess: IPSession = {
      ...
      status: 'queued',
      progress: 0,
      stats: { openPorts: 0, vulnerabilities: 0, exposures: 0 },
    };
  ...
  const runScan = useCallback((ip: string) => {
    console.log('Running full scan for:', ip);
  }, []);
  ...
  const handleSelectHistory = (historyData: IPServerData) => {
  ...
  const renderNormalContent = (reconData: IPServerData | null) => {
    if (!reconData) {
      return (
    ...
    switch (activeSubTab) {
      ...
      case 'ports':
        return <PortService data={reconData} />;
      case 'os':
        return <OSDetection data={reconData} />;
      case 'security':
        return <SecurityAnalysis data={reconData} />;
      case 'infrastructure':
        return <InfrastructureExposure data={reconData} />;
      case 'log':
