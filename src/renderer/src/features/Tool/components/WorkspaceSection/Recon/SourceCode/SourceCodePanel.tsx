...
import { SourceCodeRepoInfo } from './RepoInfo';
import { SourceCodeDeveloperInfo } from './DeveloperInfo';
import { SourceCodeSensitiveExposure } from './SensitiveExposure';
import { SourceCodeInfrastructureInfo } from './InfrastructureInfo';
import { SourceCodeAppIntelligence } from './AppIntelligence';
import { SourceCodeDependencyAnalysis } from './DependencyAnalysis';
import { TerminalLog } from './Terminal';

const SUB_TABS = [
  { id: 'overview', label: 'Overview', accent: '#0af' },
  { id: 'repo-info', label: 'Repository Information', accent: '#30d158' },
  { id: 'developer-info', label: 'Developer Information', accent: '#64d2ff' },
  { id: 'sensitive-exposure', label: 'Sensitive Exposure', accent: '#ff2d55' },
  { id: 'infrastructure-info', label: 'Infrastructure Information', accent: '#f5a623' },
  { id: 'app-intelligence', label: 'Application Intelligence', accent: '#bf5af2' },
  { id: 'dependency-analysis', label: 'Dependency Analysis', accent: '#ff6b35' },
...
export function SourceCodePanel({ activeDomain }: SourceCodePanelProps) {
  ...
  const renderContent = () => {
    if (!data) {
      return (
        <div className="flex-1 flex items-center justify-center flex-col gap-3 bg-[#080b10]">
          ...
          <div className="text-[11px] font-mono text-[#2a3548]">Enter a repository name and click "Lookup" to start reconnaissance</div>
        </div>
      );
    }
    ...
    switch (activeSubTab) {
      ...
      case 'repo-info': return <SourceCodeRepoInfo data={data} />;
      case 'developer-info': return <SourceCodeDeveloperInfo data={data} />;
      case 'sensitive-exposure': return <SourceCodeSensitiveExposure data={data} />;
      case 'infrastructure-info': return <SourceCodeInfrastructureInfo data={data} />;
      case 'app-intelligence': return <SourceCodeAppIntelligence data={data} />;
      case 'dependency-analysis': return <SourceCodeDependencyAnalysis data={data} />;
