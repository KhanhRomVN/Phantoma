...
import { PersonTechnicalFootprint } from './TechnicalFootprint';
import { PersonLeakExposure } from './LeakExposure';
import { TerminalLog } from './Terminal';

const SUB_TABS = [
  { id: 'overview', label: 'Overview', accent: '#0af' },
  { id: 'identity', label: 'Identity Information', accent: '#af52de' },
  { id: 'contact', label: 'Contact Information', accent: '#30d158' },
  { id: 'social-media', label: 'Social Media', accent: '#f5a623' },
  { id: 'technical-footprint', label: 'Technical Footprint', accent: '#bf5af2' },
  { id: 'leak-exposure', label: 'Leak & Exposure', accent: '#ff2d55' },
...
export function PersonPanel({ activeDomain }: PersonPanelProps) {
  ...
  const renderContent = () => {
    if (!data) {
      return (
        <div className="flex-1 flex items-center justify-center flex-col gap-3 bg-[#080b10]">
          ...
          <div className="text-[11px] font-mono text-[#2a3548]">Enter a person's name or email and click "Lookup" to start reconnaissance</div>
        </div>
      );
    }
    ...
    switch (activeSubTab) {
      ...
      case 'identity': return <PersonIdentity data={data} />;
      case 'contact': return <PersonContact data={data} />;
      case 'social-media': return <PersonSocialMedia data={data} />;
      case 'technical-footprint': return <PersonTechnicalFootprint data={data} />;
      case 'leak-exposure': return <PersonLeakExposure data={data} />;
