...
import { WebsiteAppStructure } from './AppStructure';
import { WebsiteAuthSurface } from './AuthSurface';
import { WebsiteClientSide } from './ClientSide';
import { WebsiteVulnerability } from './Vulnerability';
import { WebsiteTechnology } from './Technology';
import { TerminalLog } from './Terminal';

const SUB_TABS = [
  { id: 'overview', label: 'Overview', accent: '#0af' },
  { id: 'app-structure', label: 'Application Structure', accent: '#30d158' },
  { id: 'auth-surface', label: 'Authentication Surface', accent: '#f5a623' },
  { id: 'client-side', label: 'Client-Side Analysis', accent: '#64d2ff' },
  { id: 'vulnerability', label: 'Vulnerability Surface', accent: '#ff2d55' },
  { id: 'technology', label: 'Technology Detection', accent: '#bf5af2' },
...
export function WebsitePanel({ activeDomain }: WebsitePanelProps) {
  ...
  const renderContent = () => {
    if (!data) {
      return (
        <div className="flex-1 flex items-center justify-center flex-col gap-3 bg-[#080b10]">
          ...
          <div className="text-[11px] font-mono text-[#2a3548]">Enter a website URL and click "Lookup" to start reconnaissance</div>
        </div>
      );
    }
    ...
    switch (activeSubTab) {
      ...
      case 'app-structure': return <WebsiteAppStructure data={data} />;
      case 'auth-surface': return <WebsiteAuthSurface data={data} />;
      case 'client-side': return <WebsiteClientSide data={data} />;
      case 'vulnerability': return <WebsiteVulnerability data={data} />;
      case 'technology': return <WebsiteTechnology data={data} />;
