            ...
            <table className="w-full text-[10px] font-mono">
              <thead>
                <tr className="border-b border-[#1c2333] bg-[#0a0e14]">
                  ...
                  <th className="text-left p-2 text-[#2a3548] font-normal tracking-wider text-[9px] uppercase">Risk</th>
                  <th className="text-left p-2 text-[#2a3548] font-normal tracking-wider text-[9px] uppercase">CVEs</th>
                </tr>
              </thead>
              <tbody>
                {data.ports.length === 0 ? (
                  ...
                  data.ports.map((port, idx) => (
                    <tr key={idx} className="border-b border-[#111827] hover:bg-[#111827] transition-colors">
                      ...
                      <td className="p-2 text-[10px] text-[#8da0c0]">{port.service || '—'}</td>
                      <td className="p-2">
                      ...
                      <td className="p-2">
                        {port.cve && port.cve.length > 0 ? (
                          <span className="text-[9px] text-[#ff6b35]">{port.cve.join(', ')}</span>
                        ) : '—'}
                      </td>
                    </tr>
                  ))
