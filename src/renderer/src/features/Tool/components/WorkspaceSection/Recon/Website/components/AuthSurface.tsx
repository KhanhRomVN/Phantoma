    ...
    <div className="flex-1 overflow-y-auto p-3 bg-[#080b10]">
      <div className="grid grid-cols-2 gap-2">
        ...
        <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3">
          ...
          <KV k="JWT Used" v={authSurface.jwt ? 'Yes' : 'No'} vc={authSurface.jwt ? 'text-[#30d158]' : 'text-[#3a4558]'} />
            ...
            <div className="flex flex-wrap gap-2">
              {authSurface.oauth?.map((oauth, idx) => (
                <div key={idx} className="p-2 bg-[#0a0e14] rounded">
                  <div className="text-[11px] font-mono text-[#0af]">{oauth.provider}</div>
                  <div className="text-[9px] text-[#6a7a9a]">{oauth.url}</div>
                </div>
              ))}
            </div>
          </div>
