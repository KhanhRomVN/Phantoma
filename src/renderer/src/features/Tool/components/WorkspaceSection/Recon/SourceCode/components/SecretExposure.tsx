  ...
  return (
    <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3">
      ...
      {secrets.map((secret, idx) => (
        <div key={idx} className="text-[10px] font-mono py-1 border-b border-[#111827] last:border-0">
          <span className="text-[#8da0c0] break-all">{secret}</span>
        </div>
      ))}
    </div>
  );
        ...
        {infrastructureInfo.ciCdConfig && infrastructureInfo.ciCdConfig.length > 0 && (
          <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3 col-span-2">
            ...
            {infrastructureInfo.ciCdConfig.map((config, idx) => (
              <div key={idx} className="text-[10px] font-mono py-1 border-b border-[#111827] last:border-0">
                <span className="text-[#8da0c0]">{config}</span>
              </div>
            ))}
          </div>
        )}
          ...
          <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3 col-span-2">
            ...
            <div className="flex flex-wrap gap-2">
              {appIntelligence.apiEndpoints.map((endpoint, idx) => (
                <span key={idx} className="text-[9px] font-mono px-2 py-0.5 rounded bg-[#0a0e14] border border-[#1c2333] text-[#0af]">
                  {endpoint}
                </span>
              ))}
            </div>
