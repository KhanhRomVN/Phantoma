    ...
    <div className="flex-1 overflow-y-auto p-3 bg-[#080b10]">
      <div className="grid grid-cols-2 gap-2">
        ...
        <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3">
          ...
          <KV k="CDN" v={infra.cdn || 'None'} vc={infra.cdn ? 'text-[#0af]' : 'text-[#3a4558]'} />
          <KV k="WAF" v={infra.waf || 'None'} vc={infra.waf ? 'text-[#ff6b35]' : 'text-[#3a4558]'} />
          ...
          {(infra.cidrRange && infra.cidrRange.length > 0) || (infra.ipRanges && infra.ipRanges.length > 0) ? (
            <>
              ...
              {infra.ipRanges && infra.ipRanges.map((range, i) => (
                <KV key={i} k={`Range[${i}]`} v={range} vc="text-[#6a7a9a]" />
              ))}
            </>
          ) : (
        ...
        <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3">
          ...
          {infra.reverseIp && infra.reverseIp.length > 0 ? (
            infra.reverseIp.map((entry, i) => (
              <KV key={i} k={`Entry ${i + 1}`} v={entry} vc="text-[#8da0c0]" />
            ))
          ) : (
