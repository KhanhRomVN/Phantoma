...
function KV({ k, v, vc = 'text-[#6a7a9a]' }: { k: string; v: string | number | React.ReactNode; vc?: string }) {
  return (
    <div className="flex justify-between items-center py-1 border-b border-[#111827] last:border-0">
      ...
      <span className={cn('text-[11px] font-mono text-right max-w-[60%] break-all', vc)}>{v}</span>
    </div>
  );
}
...
function ConfidenceMeter({ value }: { value: number }) {
  ...
  return (
    <div className="bg-[#0d1017] border border-[#1c2333] rounded p-2.5 flex flex-col gap-1">
      <span className="text-[9px] uppercase tracking-widest font-mono text-[#3a4558]">Confidence</span>
      <div className="flex items-center gap-2">
        <span className="text-[15px] font-bold font-mono leading-none" style={{ color }}>{pct}%</span>
      </div>
...
function QueryTypeBadge({ type }: { type?: string }) {
  ...
  return (
    <span
      className="text-[9px] font-mono font-bold px-2 py-0.5 rounded tracking-widest"
    ...
    <div className="flex-1 overflow-y-auto p-3 bg-[#080b10]">
      ...
      <div className="flex items-center justify-between mb-2 px-0.5">
        <div className="flex items-center gap-2">
          ...
          {totalHits > 0 && (
            <span className="text-[9px] font-mono text-[#2a3548]">{totalHits} hits</span>
          )}
        </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3">
          ...
          {identityInfo.possibleRealNames && identityInfo.possibleRealNames.length > 0 && (
            <div className="py-1 border-b border-[#111827]">
              <span className="text-[10px] font-mono text-[#3a4558] uppercase tracking-wide block mb-1">Possible Names</span>
              <TagList items={identityInfo.possibleRealNames} color="#af52de" />
            </div>
          )}
          {identityInfo.language && identityInfo.language.length > 0 && (
            <div className="py-1 border-b border-[#111827]">
              <span className="text-[10px] font-mono text-[#3a4558] uppercase tracking-wide block mb-1">Language</span>
              <TagList items={identityInfo.language} color="#64d2ff" />
            </div>
          )}
          {identityInfo.alias && identityInfo.alias.length > 0 && (
            <div className="py-1 border-b border-[#111827]">
              <span className="text-[10px] font-mono text-[#3a4558] uppercase tracking-wide block mb-1">Aliases</span>
              <TagList items={identityInfo.alias} color="#f5a623" />
            </div>
          )}
          {identityInfo.username && identityInfo.username.length > 0 && (
            <div className="py-1">
              <span className="text-[10px] font-mono text-[#3a4558] uppercase tracking-wide block mb-1">Usernames</span>
        ...
        <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3">
          ...
          {contactInfo.email && contactInfo.email.length > 0 && (
            <div className="py-1 border-b border-[#111827]">
              <span className="text-[10px] font-mono text-[#3a4558] uppercase tracking-wide block mb-1">Email</span>
              {contactInfo.email.map((e, i) => (
                <div key={i} className="text-[11px] font-mono text-[#0af]">{e}</div>
              ))}
            </div>
          )}
        ...
        {identityInfo.notes && (
          <div className="col-span-2 bg-[#0d1017] border border-[#f5a62330] rounded p-3">
            ...
            <p className="text-[11px] font-mono text-[#8da0c0] leading-relaxed">{identityInfo.notes}</p>
          </div>
        )}
      </div>
    </div>
