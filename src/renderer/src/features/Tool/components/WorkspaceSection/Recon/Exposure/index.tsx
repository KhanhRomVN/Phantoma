import { cloudAssets, codeRepos, darkWebLeaks, googleDorks, waybackSnapshots, RISK_COLOR, SectionHeader, RiskPill } from '../shared';

export function TabExposure() {
  return (
            ...
            {cloudAssets.map((a, i) => (
              ...
              >
                ...
                <div className="space-y-0.5">
                  {a.files.map((f, fi) => (
                    <div key={fi} className="text-[8.5px] font-mono text-[#3a4558] flex items-center gap-1">
          ...
          {darkWebLeaks.map((l, i) => (
            ...
            >
              <div className="flex items-center justify-between mb-0.5">
                <span className="text-[10px] font-mono font-bold" style={{ color: RISK_COLOR[l.risk] }}>
                  {l.source}
                </span>
          ...
          <div className="grid grid-cols-3 gap-2">
            {googleDorks.map((d, i) => (
              <div key={i} className="bg-[#060810] border border-[#1c2333] rounded p-2">
                ...
                {d.urls.map((u, ui) => (
                  <div key={ui} className="text-[8.5px] text-[#0af] truncate">› {u}</div>
                ))}
              </div>
            ))}
          </div>
