              ...
              <table className="w-full text-[10px] font-mono">
                <thead>
                  <tr className="border-b border-[#1c2333] bg-[#0a0e14]">
                    <th className="text-left p-2 text-[#2a3548] font-normal tracking-wider text-[9px] uppercase">{type.replace(/([A-Z])/g, ' $1').trim()}</th>
                ...
                <tbody>
                  {assets.map((asset, idx) => (
                    <tr key={idx} className="border-b border-[#111827] hover:bg-[#111827] transition-colors">
                      <td className="p-2 text-[10px] text-[#6a7a9a]">{asset.type}</td>
                      <td className="p-2 font-mono text-[11px] text-[#0af]">{asset.name}</td>
