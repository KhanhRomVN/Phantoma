...
import { SectionHeader, StatBox, BreachTimeline } from '../../shared-ui';
import type { ReconData } from '../ReconDataContext';

export function TabBreach({ data }: { data: ReconData }) {
  const breaches = data.breaches;
  const harvestedEmails = data.harvestedEmails;
            ...
            {['emails', 'passwords', 'ips', 'payment', 'hints'].map((cat) => {
              ...
              return (
                <div key={cat} className="flex items-center gap-2">
                  <span className="text-[10px] font-mono text-[#3a4558] w-20">{cat}</span>
                  ...
                  <span className="text-[10px] text-[#4a5a7a] font-mono">{count}</span>
                </div>
              );
            })}
          </div>
            ...
            <thead>
              <tr className="border-b border-[#1c2333]">
                {['Email', 'Role', 'Source', 'Verified', 'In Breach'].map((h) => (
                  <th
                    key={h}
                    className="text-left p-1.5 text-[#2a3548] font-normal text-[10px] uppercase tracking-wider"
                  >
                    {h}
                  </th>
                ))}
