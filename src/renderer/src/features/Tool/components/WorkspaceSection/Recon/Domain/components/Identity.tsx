...
function KV({ k, v, vc = 'text-[#6a7a9a]' }: { k: string; v: string | number | ReactNode; vc?: string }) {
  return (
...
function ExpandableRaw({ content }: { content: string }) {
  ...
  return (
    <div className="mt-2">
      <pre className="text-[9px] font-mono text-[#5a6a8a] bg-[#0a0e14] p-2 rounded overflow-x-auto whitespace-pre-wrap">
        {expanded ? content : preview}
      </pre>
      {content.split('\n').length > 3 && (
        <button
          onClick={() => setExpanded(!expanded)}
          className="text-[9px] font-mono text-[#0af] hover:text-[#5cf] mt-1"
        >
        ...
        <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3 col-span-2">
          ...
          <div className="flex flex-wrap gap-2">
            {(identity.nameservers || identity.nameServers || []).map((ns: string, idx: number) => (
              <span key={idx} className="text-[10px] font-mono text-[#8da0c0] bg-[#0a0e14] px-2 py-0.5 rounded border border-[#1c2333]">
                {ns}
              </span>
            ))}
          </div>
          ...
          <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3 col-span-2">
            ...
            <div className="flex flex-wrap gap-2">
              {identity.domainStatus.map((status: string, idx: number) => (
                <span key={idx} className="text-[11px] font-mono px-2 py-1 rounded" style={{ background: '#bf5af215', border: '1px solid #bf5af230', color: '#bf5af2' }}>
                  {status}
                </span>
              ))}
            </div>
