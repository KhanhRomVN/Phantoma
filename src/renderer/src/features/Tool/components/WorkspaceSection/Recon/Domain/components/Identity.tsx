  ...
  return (
    <div className="flex-1 overflow-y-auto p-3 bg-[#080b10]">
      <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3">
        <SectionHeader accent="#0af">Domain Identity</SectionHeader>
        <KV k="Domain" v={whois.domain} />
        <KV k="Registrar" v={whois.registrar} />
        <KV k="Creation Date" v={whois.creationDate} />
        <KV k="Expiration Date" v={whois.expirationDate} />
        <KV k="Name Servers" v={whois.nameServers.join(', ')} />
        <KV k="DNSSEC" v={whois.dnssec} />
      </div>
    </div>
  );
}