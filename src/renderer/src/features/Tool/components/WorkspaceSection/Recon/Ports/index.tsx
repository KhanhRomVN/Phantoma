...
import { RISK_COLOR, SectionHeader, RiskPill } from '../shared';
...
function PhaseBadge({ phase, tool, success, error }: Partial<PhaseResult>) {
  const meta = PHASE_META[phase as keyof typeof PHASE_META] ?? { label: phase, accent: '#4a5a7a', icon: '·' };
      ...
      <thead>
        <tr className="border-b border-[#1c2333]">
          {['Port', 'Proto', 'Service', 'Product', 'State', 'Risk', 'CVEs'].map((h) => (
            <th key={h} className="text-left p-1.5 text-[#2a3548] font-normal tracking-wider text-[9px] uppercase">
              {h}
            </th>
          ))}
        </tr>
      ...
      <tbody>
        {ports.map((p, i) => (
          <tr key={i} className="border-b border-[#0d1017] hover:bg-[#111827] transition-colors">
            <td className="p-1.5 font-bold" style={{ color: riskColor(p.risk) }}>{p.port}</td>
            ...
            <td className="p-1.5">
              <span className={p.state === 'open' ? 'text-[#30d158]' : 'text-[#3a4558]'}>{p.state}</span>
            </td>
            <td className="p-1.5"><RiskPill level={p.risk} /></td>
  ...
  return (
    <div className="space-y-1.5">
      {vulns.map((v, i) => {
        ...
        return (
          <div key={i} className="border rounded p-2" style={{ borderColor: `${c}25`, background: `${c}06` }}>
            <div className="flex items-center justify-between mb-0.5">
              <div className="flex items-center gap-2">
                <span className="font-mono text-[10px] font-bold" style={{ color: c }}>{v.name}</span>
                {v.id && <span className="text-[8px] font-mono text-[#3a4558]">{v.id}</span>}
              </div>
              <div className="flex items-center gap-1.5">
                <span className="text-[8px] font-mono text-[#3a4558] bg-[#111827] px-1.5 py-0.5 rounded">{v.tool}</span>
                <span className="text-[8px] font-bold font-mono uppercase px-1.5 py-0.5 rounded-sm"
                  style={{ color: c, border: `1px solid ${c}40`, background: `${c}12` }}>
                  {v.severity}
                </span>
              </div>
            </div>
            {v.url && <div className="text-[8px] font-mono text-[#0af] mb-0.5 truncate">{v.url}</div>}
            {v.description && <div className="text-[9px] text-[#6a7a9a] leading-relaxed">{v.description}</div>}
          </div>
        );
      })}
    </div>
...
function PhaseCard({ phase }: { phase: PhaseResult }) {
  ...
  const meta = PHASE_META[phase.phase as keyof typeof PHASE_META] ?? { label: phase.phase, accent: '#4a5a7a', icon: '·' };
  ...
  return (
    <div className="bg-[#0d1017] border border-[#1c2333] rounded overflow-hidden">
      ...
      >
        ...
        <span className="text-[9px] font-bold font-mono uppercase tracking-widest" style={{ color: meta.accent + 'bb' }}>
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          {phase.error && (
            <span className="text-[9px] font-mono text-[#ff2d55] truncate max-w-xs">{phase.error}</span>
          )}
          {hasContent && (
...
// ── Scan form ─────────────────────────────────────────────────────────────────
function ScanForm({ onScan, scanning, phase }: {
  ...
  return (
    <form onSubmit={handleSubmit} className="col-span-2 bg-[#0d1017] border border-[#1c2333] rounded p-3 flex flex-col gap-3">
      {/* Target row */}
      <div className="flex items-center gap-2">
        ...
        <button
          type="submit" disabled={scanning || !target.trim()}
        ...
        >
          {scanning ? (
            <><span className="w-2 h-2 rounded-full border border-current border-t-transparent animate-spin" /> {phase}</>
          ) : '▶ Run Pipeline'}
        </button>
      </div>
    </form>
...
export function TabPorts() {
  ...
  const runScan = async (target: string, ports: string, phases: string[]) => {
    ...
    const serverUrl = (localStorage.getItem('phantoma_server_url') ?? 'http://localhost:8080')
      .replace(/\/$/, '');
    ...
    try {
      ...
      try {
        ...
        setState({ status: 'error', message: `JSON parse error: ${e}\n\nRaw: ${text.slice(0, 300)}` });
        return;
      }
  ...
  return (
    <div className="flex-1 overflow-y-auto p-3 bg-[#080b10]">
      <div className="grid grid-cols-2 gap-2">
        ...
        {/* Error */}
        {state.status === 'error' && (
          <div className="col-span-2 flex items-start gap-2 px-3 py-2 rounded text-[9px] font-mono whitespace-pre-wrap"
          ...
          <div className="col-span-2 flex flex-col items-center justify-center py-16 gap-3">
            ...
            <div className="flex gap-2">
              {[
                ...
                <span key={p.id} className="text-[8px] font-mono px-2 py-0.5 rounded"
          ...
          <>
            ...
            <div className="col-span-2 grid grid-cols-5 gap-2">
              {[
                ...
                <div key={label} className="p-2.5 bg-[#0d1017] border border-[#1c2333] rounded flex flex-col gap-0.5">
                  <span className="text-[8px] uppercase tracking-widest font-mono text-[#3a4558]">{label}</span>
                  <span className="text-[15px] font-bold font-mono leading-none truncate" style={{ color: accent }}>
                    {value}
                  </span>
                </div>
              ))}
