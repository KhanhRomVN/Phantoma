...
export function TabIntel({ data }: { data: ReconData }) {
  const { threatIntel, socialIntel, certTransparency, httpHeaders, whoisData } = data;
    ...
    <div className="flex-1 overflow-y-auto p-3 bg-[#080b10]">
      <div className="grid grid-cols-2 gap-2">
              ...
              {threatIntel.map((ti, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="p-1.5">
                      <span className="uppercase font-bold text-[10px]" style={{ color: vc }}>
                        {ti.verdict}
                      </span>
                    </td>
            ...
            {socialIntel.map((s, i) => (
              <div key={i} className="bg-[#060810] border border-[#1c2333] rounded p-2.5">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[11px] font-semibold text-[#8da0c0]">{s.name}</span>
                </div>
                <div className="text-[10px] text-[#3a4558] font-mono mb-1">{s.role}</div>
                <div className="text-[10px] text-[#6a7a9a] mb-1">{s.intel}</div>
                <div className="text-[9px] text-[#2a3548] font-mono">{s.url}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          {certTransparency.map((cert, i) => (
            ...
            >
              <div className="flex items-center justify-between mb-1">
                <span className="font-mono text-[11px] font-bold text-[#0af]">
                  {cert.commonName}
                </span>
                <div className="flex items-center gap-2">
                  <span className="text-[9px] text-[#2a3548] font-mono">
              ...
              <div className="text-[10px] text-[#3a4558] mb-1">{cert.issuer}</div>
              <div className="flex flex-wrap gap-1">
                {cert.san.map((s) => (
                  <span
                    key={s}
                    className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-[#0af10] border border-[#0af20] text-[#0af]"
                  >
                    {s}
                  </span>
                ))}
        ...
        <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3">
          ...
          <div className="mb-3">
            <div className="text-[9.5px] uppercase tracking-wider text-[#2a3548] font-mono mb-1">
              Disallowed Paths
            </div>
            {['/admin', '/api/private', '/backup', '/jenkins', '/internal'].map((p) => (
              <div
                key={p}
                className="text-[10px] font-mono text-[#ff6b35] flex items-center gap-1 mb-0.5"
              >
          ...
          <div>
            ...
            {['/', 'about', 'contact', 'blog', 'privacy', 'terms', 'admin/login', 'api/docs'].map(
              (p) => (
                <div
                  key={p}
                  className="text-[10px] font-mono text-[#4a5a7a] flex items-center gap-1 mb-0.5"
                >
