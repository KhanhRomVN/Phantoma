/**
 * usePersonRecon — main hook for processing Person/Email RECON data.
 */
...
import { parsePersonReconResult } from '../utils/person/person-parser';

interface UsePersonReconReturn {
...
export function usePersonRecon(): UsePersonReconReturn {
  ...
  const loadData = useCallback((rawData: Record<string, unknown>) => {
    ...
    setError(null);
    try {
      const parsed = parsePersonReconResult(rawData);
      setResult(parsed);
      const primary = parsed.entities.find(e => e.relevance === 'primary') || parsed.entities[0] || null;
      setSelectedEntityId(primary?.id || null);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to parse Person RECON data');
      setResult(null);
    } finally {
      setIsLoading(false);
    }
