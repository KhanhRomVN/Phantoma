...
export function SocialMedia({ dataPoints, activeGroup }: SocialMediaProps) {
  const confirmedProfiles = dataPoints.filter(
    (dp) => dp.verificationStatus === 'verified' || dp.tags?.includes('confirmed'),
  ).length;
  const noiseProfiles = dataPoints.filter(
    (dp) => dp.isNoise || dp.tags?.includes('collision'),
  ).length;
  ...
  return (
    <div className="flex-1 overflow-y-auto p-3">
      <div className="grid grid-cols-3 gap-2 mb-3">
        ...
        <StatBox label="Confirmed" value={confirmedProfiles} sub="verified" accent="#30d158" />
        <StatBox
          label="Noise"
          value={noiseProfiles}
          sub="collisions"
      ...
      <SectionHeader accent="#ff6b35">
        Social Media Profiles ({uniquePlatforms} platforms)
      </SectionHeader>
      <DataTable
