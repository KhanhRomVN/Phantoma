...
export function ReverseIp({ dataPoints, activeGroup }: ReverseIpProps) {
  ...
  const unrelatedDomains = dataPoints.filter(
    (dp) =>
      ['unrelated_domain', 'hosted_domain'].includes(dp.category) &&
      !dp.tags?.includes('target-domain'),
  ).length;
  ...
  return (
    <div className="flex-1 overflow-y-auto p-3">
      <div className="grid grid-cols-3 gap-2 mb-3">
        ...
        <StatBox label="Unrelated" value={unrelatedDomains} sub="shared IP" accent="#6a7a9a" />
      </div>
