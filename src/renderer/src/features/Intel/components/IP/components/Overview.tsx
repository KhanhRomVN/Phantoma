...
export function Overview({ result, onSelectEntity }: OverviewProps) {
  ...
  const openPortsCount = result.allDataPoints.filter((dp) => dp.category === 'open_port').length;
  const threatCount = result.allDataPoints.filter((dp) =>
    ['threat_report', 'malware_association', 'phishing_association', 'c2_communication', 'brute_force'].includes(dp.category),
  ).length;
  const hostedDomainsCount = result.allDataPoints.filter((dp) =>
    ['hosted_domain', 'primary_domain', 'related_domain'].includes(dp.category),
  ).length;
  ...
  return (
    <div className="flex-1 overflow-y-auto p-3">
      ...
      <div className="grid grid-cols-4 gap-2 mb-3">
        ...
        <StatBox label="Open Ports" value={openPortsCount} sub="detected" accent="#ff6b35" />
        <StatBox label="Domains" value={hostedDomainsCount} sub="hosted" accent="#0a84ff" />
        <StatBox label="Threats" value={threatCount} sub="reported" accent={threatCount > 0 ? '#ff2d55' : '#30d158'} />
      </div>
      ...
      <div className="bg-[#0d1017] border border-[#1c2333] rounded p-3 mb-3">
        ...
        <div className="flex justify-between items-center py-1 border-b border-[#111827]">
          <span className="text-[11px] font-mono text-[#6a7a9a] uppercase tracking-wide">IP Address</span>
          <span className="text-[12px] font-mono text-[#0af]">{result.query.value}</span>
        </div>
        {result.targetDomain && (
          <div className="flex justify-between items-center py-1 border-b border-[#111827]">
            <span className="text-[11px] font-mono text-[#6a7a9a] uppercase tracking-wide">Target Domain</span>
            <span className="text-[12px] font-mono text-[#c8d6f0]">{result.targetDomain}</span>
          </div>
        )}
        <div className="flex justify-between items-center py-1 border-b border-[#111827]">
