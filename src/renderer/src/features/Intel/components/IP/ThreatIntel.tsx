...
export function ThreatIntel({ dataPoints, activeGroup }: ThreatIntelProps) {
  const confirmedThreats = dataPoints.filter(
    (dp) => !dp.isNoise && !dp.tags?.includes('false-positive'),
  ).length;
  const falsePositives = dataPoints.filter(
    (dp) => dp.isNoise || dp.tags?.includes('false-positive'),
  ).length;
  const criticalThreats = dataPoints.filter(
    (dp) => dp.tags?.includes('critical-risk') || dp.category === 'darkweb_mention',
  ).length;
