    ...
    <div className="flex h-full bg-[#0f1319]">
        ...
        <div className="flex h-10 border-b border-[#1e2535] shrink-0 overflow-x-auto">
          {targetTabs.map((tab, index) => (
            <button
              key={tab.id}
              onClick={() => {
                console.log('[DEBUG Emulate] Tab clicked:', {
                  id: tab.id,
                  title: tab.title,
                ...
                if (tab.title === 'Chưa chọn target') {
                  console.log(
                    '[DEBUG Emulate] Setting showTargetPanel to true for tab:',
                    tab.title,
                  );
                  setShowTargetPanel(true);
                } else {
                  console.log(
                    '[DEBUG Emulate] Setting showTargetPanel to false for tab:',
                    tab.title,
                  );
        ...
        activeTargetId === 'default' ? (
          <TargetPanel
            ...
            onTargetSelected={(target) => {
              console.log('[DEBUG Emulate] Target selected:', {
                target,
                activeTargetId,
                currentTabs: targetTabs.map((t) => ({ id: t.id, title: t.title })),
              });
                ...
                const newTabs = prev.map((tab) =>
                  tab.id === activeTargetId
                    ? {
                        ...
                        title: target.name,
                        favicon: target.url
                          ? `https://www.google.com/s2/favicons?domain=${new URL(target.url).hostname}&sz=32`
                          : undefined,
                        url: target.url,
                      }
                    : tab,
              ...
              setShowTargetPanel(false);
              console.log(
                '[DEBUG Emulate] After update - activeTargetId set to:',
                target.id,
                'showTargetPanel set to false',
              );
            }}
          />
        ) : (
      ...
      {(() => {
        const shouldShowToolbar =
          activeTargetId &&
          targetTabs.find((tab) => tab.id === activeTargetId)?.title !== 'Chưa chọn target' &&
          targetTabs.length > 0;
