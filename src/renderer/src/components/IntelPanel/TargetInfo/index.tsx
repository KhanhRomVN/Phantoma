...
function riskLabel(score: number) {
  ...
  if (score >= 60)
    return { label: 'HIGH', cls: 'text-orange-400 border-orange-500/30 bg-orange-500/8' };
  if (score >= 40)
    return { label: 'MEDIUM', cls: 'text-amber-400 border-amber-500/30 bg-amber-500/8' };
...
export function TargetInfo({ subTarget }: { subTarget: SubTarget }) {
  ...
  return (
    <div className="flex flex-col overflow-hidden border-t border-divider h-1/3">
      {/* header */}
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2.5 bg-background [&::-webkit-scrollbar]:w-[3px] [&::-webkit-scrollbar-thumb]:bg-border">
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[9.5px] shrink-0 leading-none">
            {TYPE_ICON[subTarget.type] ?? '🎯'}
          </span>
          ...
          {risk && (
            <span
              className={cn('text-[9px] font-bold px-1.5 py-0.5 rounded border shrink-0', risk.cls)}
            >
              {risk.label}
            </span>
          )}
        ...
        <div className="grid grid-cols-2 gap-1.5 text-[9.5px]">
          <div className="bg-card-background border border-divider rounded px-2 py-1.5">
            <div className="text-[8.5px] text-text-secondary uppercase tracking-wide mb-0.5">
              Type
            </div>
          ...
          <div className="bg-card-background border border-divider rounded px-2 py-1.5">
            ...
            <div className={cn('capitalize', STATUS_COLOR[subTarget.status])}>
              {subTarget.status}
            </div>
          </div>
          {subTarget.riskScore !== undefined && (
            <div className="bg-card-background border border-divider rounded px-2 py-1.5">
              ...
              <div className={cn('font-bold', risk?.cls.split(' ')[0])}>
                {subTarget.riskScore}/100
              </div>
            </div>
          )}
          {subTarget.id && (
        ...
        {subTarget.tags && subTarget.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-2">
            {subTarget.tags.map((tag) => (
              <span
                key={tag}
                className="text-[8.5px] px-1.5 py-0.5 rounded bg-sidebar-item-hover border border-border text-text-secondary"
              >
                {tag}
              </span>
            ))}
