...
import DeleteRepeaterBlock from '../../blocks/emulate/DeleteRepeaterBlock';
import ErrorBlock from '../../blocks/other/ErrorBlock';

export const DeleteRepeaterRenderer: React.FC<BaseRendererProps> = ({
  actionIndex,
  messageId,
  isActionClicked,
  isActiveGroup,
  ...
  return (
    <div className={cn('flex flex-col gap-1.5 pb-1', isLastItemInList ? 'mb-0' : 'mb-0.5')}>
      <TagHeader
        title={
          <div className="flex items-center gap-2 text-xs text-text-primary">
            <span className="font-semibold opacity-80">{getToolLabel('delete_repeater')}</span>
            {!isCompleted && (
      ...
      {output && !isError && !isCollapsed && (
        <DeleteRepeaterBlock content={output} maxHeight="200px" />
      )}
    </div>
  );
};