...
const FileStreamingBlock: React.FC<FileStreamingBlockProps> = ({
  ...
  return (
    <>
      <div
        ref={containerRef}
        className={`file-streaming-block overflow-y-auto overflow-x-hidden bg-[var(--vscode-editor-background,var(--vscode-textCodeBlock-background))] border border-[var(--vscode-widget-border,rgba(255,255,255,0.08))] !border-t-0 rounded-b-[4px] my-1 mx-3 mb-3 ml-[29px] px-2.5 py-1.5 font-mono text-[11px] leading-[1.5] text-[var(--vscode-editor-foreground)] whitespace-pre break-all opacity-85 relative shadow-[inset_0_8px_8px_-8px_rgba(0,0,0,0.3)] ${className}`}
        style={{
      ...
      <style>{`
        ...
        .file-streaming-block::-webkit-scrollbar-thumb:hover {
          background: var(--vscode-scrollbarSlider-hoverBackground, rgba(100, 100, 100, 0.5));
        }
