...
export const ErrorRenderer: React.FC<ErrorRendererProps> = ({
  ...
  const translateError = (raw: string): string => {
    ...
    if (/file.*path.*required|missing file path/i.test(normalized)) return 'File path is required.';
    if (/folder.*path.*required/i.test(normalized)) return 'Folder path is required.';
    ...
    if (/search text not found/i.test(normalized)) return 'Search text not found in file.';
    if (/no change made/i.test(normalized)) return 'No changes were made.';
    if (/command validation failed/i.test(normalized)) return 'Command validation failed.';
  ...
  const errorLabel = toolName ? toolName.toUpperCase().replace(/_/g, ' ') : label || 'ERROR';
