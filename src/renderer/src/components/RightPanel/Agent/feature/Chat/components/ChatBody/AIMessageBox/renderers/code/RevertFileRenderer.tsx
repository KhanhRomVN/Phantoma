      ...
      <TagHeader
        title={
          <div className="flex items-center gap-2 text-xs text-text-primary">
            ...
            {diffStats && (
              <>
                <span className="text-success font-semibold text-[11px]">+{diffStats.added}</span>
                <span className="text-error font-semibold text-[11px]">-{diffStats.removed}</span>
              </>
            )}
            {isCompleted && !isError && (
              <span className="text-[10px] opacity-50 text-text-secondary">reverted</span>
            )}
          </div>
        }
        path={rawPath}
