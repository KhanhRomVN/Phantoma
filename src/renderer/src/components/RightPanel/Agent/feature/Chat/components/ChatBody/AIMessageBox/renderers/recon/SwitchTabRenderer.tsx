...
export const SwitchTabRenderer: React.FC<BaseRendererProps> = ({
  action,
  actionIndex,
  messageId,
  isLastItemInList,
            ...
            <div className="flex items-start gap-2 flex-nowrap">
              ...
              <div className="flex-1 min-w-0 flex flex-col gap-0.5 mt-0.5">
                <div className="flex items-center gap-2 text-xs">
                  <span className="font-semibold text-text-primary opacity-80">SWITCH TAB</span>
                  {action.params.tabId && (
...
function SwitchTabBlock({ tabId, targetId, output, isError }: SwitchTabBlockProps) {
  return (
    <div className="text-xs space-y-2">
      ...
      {!output && !isError && (
        <div className="text-text-secondary opacity-60">Switching tab...</div>
      )}
    </div>
  );
}