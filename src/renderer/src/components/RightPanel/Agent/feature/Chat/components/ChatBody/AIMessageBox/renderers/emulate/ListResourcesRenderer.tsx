...
export const ListResourcesRenderer: React.FC<BaseRendererProps> = ({
  ...
  if (output && !isError) {
    ...
    if (match) {
      resourceCount = parseInt(match[1], 10);
    }
  }
      ...
      <TagHeader
        title={
          <div className="flex items-center gap-2 text-xs text-text-primary">
            <span className="font-semibold opacity-80">{getToolLabel('list_resources')}</span>
            {isCompleted && !isError && resourceCount > 0 && (
