...
export const useToolExecution = ({
  ...
  // Build executor context for tool execution
  ...
  const executeSingleAction = useCallback(
    async (
      ...
      return executor.execute(action, context, { skipDiagnostics, bypassIgnore });
    },
  ...
  const confirmSingleLineAction = useCallback(
    ...
    [singleLineReviewActions, messagesRef, executeSingleAction],
  );
  ...
  const rejectSingleLineAction = useCallback(
    (actionId: string) => {
      ...
      setAvailableToolResultsBuffer((prev) => {
        ...
        const isAllComplete =
          allActionIds.every(
            (id: string) => clickedActionsRef.current.has(id) || id === actionId,
          ) && isQuestionAnswered;
