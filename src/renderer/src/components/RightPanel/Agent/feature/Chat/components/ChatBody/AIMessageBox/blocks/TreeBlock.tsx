...
const TreeNode: React.FC<{
  ...
  return (
    <div className="tree-node">
      <div
        className={`tree-node-content flex items-center gap-1 py-0.5 text-[13px] ${node.type === 'file' ? 'cursor-pointer hover:bg-sidebar-item-hover rounded-sm' : ''}`}
        ...
        <img
          ...
          onError={(e) => {
            ...
            if (parent) {
              ...
              fallback.className = 'codicon codicon-' + (node.type === 'folder' ? 'folder' : 'file') + ' text-xs text-text-secondary opacity-70 mr-1.5 shrink-0';
              fallback.style.cssText = '';
              parent.insertBefore(fallback, e.currentTarget);
            }
          }}
        />
        <span className="tree-node-name text-text-primary">{node.name}</span>
        {node.type === 'file' && node.lines !== undefined && (
          <span className="tree-node-meta text-[11px] text-secondary opacity-60 ml-auto mr-2">
            {node.lines} lines
          </span>
        )}
      </div>
