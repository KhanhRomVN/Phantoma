...
import { ExecutableToolType, getAllToolTypes } from '../constants/tool-registry';
...
export const parseAIResponse = (content: string): ParsedResponse => {
  ...
  const toolPatterns = [
    ...getAllToolTypes().filter((t: string) => t !== 'thinking'), // Exclude thinking (pre-extracted)
  ...
  const findNextTag = (str: string) => {
    ...
    for (const toolName of toolPatterns) {
      ...
      const closingPos = findClosingTagPosition(str, startContentPos, closingTagPattern);
    ...
    if (index !== -1 && match) {
      ...
      if (isClosed) {
        ...
        const extractSubTags = (content: string): string[] => {
          const subTagRegex = /<([a-zA-Z_][a-zA-Z0-9_]*?)(?:\s+[^>]*)?>[\s\S]*?<\/\1>/g;
