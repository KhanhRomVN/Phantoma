/**
 * ------------------------------------------------------------------
 * Tool Executor Index
 * ------------------------------------------------------------------
 * Central registry mapping tool names → executor functions.
 * Factory function getExecutor() trả về executor tương ứng.
 *
 * Main functions:
 * - getExecutor() : Factory trả về executor cho một tool type
 * ------------------------------------------------------------------
 */

// ─── Re-exports ─────────────────────────────────────────────────────────
// Re-export — Code tools
export {
  executeReadFile,
  executeWriteToFile,
  executeReplaceInFile,
  executeRevertFile,
  executeViewReplaceHistory,
  executeListFiles,
  executeFindFiles,
  executeRunCommand,
  executeDeleteFile,
  executeGrep,
  executeGitDiff,
  executeMoveFile,
} from './CodeExecutor';

// ═══════════════════════════════════════════════════════════════════════
// Re-export — Emulate tools
// ═══════════════════════════════════════════════════════════════════════
export {
  executeListHttps,
  executeListHosts,
  executeListSources,
  executeGetSourceDetail,
  executeGetHttpsDetail,
  executeApplyFilter,
} from './EmulateExecutor';

// ═══════════════════════════════════════════════════════════════════════
// Re-export — Recon tools
// ═══════════════════════════════════════════════════════════════════════
export {
  executeListTabs,
  executeCreateTab,
  executeCloseTab,
  executeSwitchTab,
  executeNavigate,
  executeBack,
  executeForward,
  executeReload,
  executeGetPageContent,
  executeListElements,
  executeClickElement,
  executeFillInput,
  executePressKey,
  executeScroll,
} from './ReconExecutor';

import { logger } from '@renderer/utils/logger';
// ═══════════════════════════════════════════════════════════════════════
// Imports
// ═══════════════════════════════════════════════════════════════════════

import type { ExecutorContext, ExecutorOptions, ToolExecutor } from '../../types/executor-types';

// ── Code tool executors ──────────────────────────────────────────────
import {
  executeReadFile,
  executeWriteToFile,
  executeReplaceInFile,
  executeRevertFile,
  executeViewReplaceHistory,
  executeListFiles,
  executeFindFiles,
  executeRunCommand,
  executeDeleteFile,
  executeGrep,
  executeGitDiff,
  executeMoveFile,
} from './CodeExecutor';

// ── Emulate tool executors ──────────────────────────────────────────
import {
  executeListHttps,
  executeListHosts,
  executeListSources,
  executeListResources,
  executeGetSourceDetail,
  executeGetResourceContent,
  executeGetHttpsDetail,
  executeApplyFilter,
  executeSendToRepeater,
  executeListRepeaters,
  executeDeleteRepeater,
  executeGetRepeaterDetail,
  executeUpdateRepeaterContent,
  executeRunRepeater,
} from './EmulateExecutor';

// ── Recon tool executors ────────────────────────────────────────────
import {
  executeListTabs,
  executeCreateTab,
  executeCloseTab,
  executeSwitchTab,
  executeNavigate,
  executeBack,
  executeForward,
  executeReload,
  executeGetPageContent,
  executeListElements,
  executeClickElement,
  executeFillInput,
  executePressKey,
  executeScroll,
} from './ReconExecutor';

// ═══════════════════════════════════════════════════════════════════════
// Factory
// ═══════════════════════════════════════════════════════════════════════

/**
 * Factory function — mỗi case gọi executor function tương ứng.
 * Tất cả executor đã được refactor để gọi Controller.executeTool() thay vì IPC.
 */
export function getExecutor(actionType: string): ToolExecutor | null {
  switch (actionType) {
    // ── Code tools ──────────────────────────────────────────────
    case 'read_file':
      return {
        execute: async (action: any, _ctx: ExecutorContext, options?: ExecutorOptions) => {
          const result = await executeReadFile(action.params, options?.bypassIgnore ?? false);
          return result?.output || null;
        },
      };
    case 'write_to_file':
      return {
        execute: async (action: any, ctx: ExecutorContext, options?: ExecutorOptions) => {
          return executeWriteToFile(
            action.params,
            options?.skipDiagnostics ?? false,
            options?.bypassIgnore ?? false,
            ctx.conversationIdRef?.current,
            (action as any).actionId,
          );
        },
      };
    case 'replace_in_file':
      return {
        execute: async (action: any, ctx: ExecutorContext, options?: ExecutorOptions) => {
          return executeReplaceInFile(
            action.params,
            options?.skipDiagnostics ?? false,
            options?.bypassIgnore ?? false,
            ctx.conversationIdRef?.current,
            (action as any).actionId,
          );
        },
      };
    case 'revert_file':
      return {
        execute: async (action: any, ctx: ExecutorContext, _options?: ExecutorOptions) => {
          return executeRevertFile(
            action.params,
            false,
            ctx.conversationIdRef?.current,
            (action as any).actionId,
          );
        },
      };
    case 'view_replace_history':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          return executeViewReplaceHistory(action.params);
        },
      };
    case 'list_files':
      return {
        execute: async (action: any, _ctx: ExecutorContext, options?: ExecutorOptions) => {
          return executeListFiles(action.params, options?.bypassIgnore ?? false);
        },
      };
    case 'find_files':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          const result = await executeFindFiles(action.params);
          return result?.output || null;
        },
      };
    case 'run_command':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          const cmdId = (action as any).actionId || 'cmd-' + Date.now();
          return executeRunCommand(action.params, cmdId);
        },
      };
    case 'delete_file':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          return executeDeleteFile(action.params);
        },
      };
    case 'grep':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          return executeGrep(action.params);
        },
      };
    case 'git_diff':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          const filePath =
            typeof action.params === 'string'
              ? action.params
              : action.params?.path || action.params?.file_path || '';
          const diffId = (action as any).actionId || 'git-diff-' + Date.now();
          return executeGitDiff(filePath, diffId);
        },
      };
    case 'move_file':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          return executeMoveFile(action.params);
        },
      };

    // ── Emulate tools ────────────────────────────────────────────
    case 'list_https':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          return executeListHttps(action.params || {});
        },
      };
    case 'list_hosts':
      return {
        execute: async (_action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          return executeListHosts();
        },
      };
    case 'list_sources':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          return executeListSources(action.params || {});
        },
      };
    case 'list_resources':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          return executeListResources(action.params || {});
        },
      };
    case 'get_source_detail':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          return executeGetSourceDetail(action.params || {});
        },
      };
    case 'get_resource_content':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          return executeGetResourceContent(action.params || {});
        },
      };
    case 'get_https_detail':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          return executeGetHttpsDetail(action.params || {});
        },
      };
    case 'apply_filter':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          return executeApplyFilter(action.params || {});
        },
      };
    case 'send_to_repeater':
      return {
        execute: async (action: any, ctx: ExecutorContext, _options?: ExecutorOptions) => {
          return executeSendToRepeater(action.params || {}, ctx.activeTargetId);
        },
      };
    case 'list_repeaters':
      return {
        execute: async (_action: any, ctx: ExecutorContext, _options?: ExecutorOptions) => {
          return executeListRepeaters(ctx.activeTargetId);
        },
      };
    case 'delete_repeater':
      return {
        execute: async (action: any, ctx: ExecutorContext, _options?: ExecutorOptions) => {
          return executeDeleteRepeater(action.params || {}, ctx.activeTargetId);
        },
      };
    case 'get_repeater_detail':
      return {
        execute: async (action: any, ctx: ExecutorContext, _options?: ExecutorOptions) => {
          return executeGetRepeaterDetail(action.params || {}, ctx.activeTargetId);
        },
      };
    case 'update_repeater_content':
      return {
        execute: async (action: any, ctx: ExecutorContext, _options?: ExecutorOptions) => {
          return executeUpdateRepeaterContent(action.params || {}, ctx.activeTargetId);
        },
      };
    case 'run_repeater':
      return {
        execute: async (action: any, ctx: ExecutorContext, _options?: ExecutorOptions) => {
          return executeRunRepeater(action.params || {}, ctx.activeTargetId);
        },
      };

    // ── Recon tools ──────────────────────────────────────────────
    case 'list_tabs':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          const result = await executeListTabs(action.params || {});
          return result.success ? result.output || null : result.error || null;
        },
      };
    case 'create_tab':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          const result = await executeCreateTab(action.params || {});
          return result.success ? result.output || null : result.error || null;
        },
      };
    case 'close_tab':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          const result = await executeCloseTab(action.params || {});
          return result.success ? result.output || null : result.error || null;
        },
      };
    case 'switch_tab':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          const result = await executeSwitchTab(action.params || {});
          return result.success ? result.output || null : result.error || null;
        },
      };
    case 'navigate':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          const result = await executeNavigate(action.params || {});
          return result.success ? result.output || null : result.error || null;
        },
      };
    case 'back':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          const result = await executeBack(action.params || {});
          return result.success ? result.output || null : result.error || null;
        },
      };
    case 'forward':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          const result = await executeForward(action.params || {});
          return result.success ? result.output || null : result.error || null;
        },
      };
    case 'reload':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          const result = await executeReload(action.params || {});
          return result.success ? result.output || null : result.error || null;
        },
      };
    case 'get_page_content':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          const result = await executeGetPageContent(action.params || {});
          return result.success ? result.output || null : result.error || null;
        },
      };
    case 'list_elements':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          const result = await executeListElements(action.params || {});
          return result.success ? result.output || null : result.error || null;
        },
      };
    case 'click_element':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          const result = await executeClickElement(action.params || {});
          return result.success ? result.output || null : result.error || null;
        },
      };
    case 'fill_input':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          const result = await executeFillInput(action.params || {});
          return result.success ? result.output || null : result.error || null;
        },
      };
    case 'press_key':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          const result = await executePressKey(action.params || {});
          return result.success ? result.output || null : result.error || null;
        },
      };
    case 'scroll':
      return {
        execute: async (action: any, _ctx: ExecutorContext, _options?: ExecutorOptions) => {
          const result = await executeScroll(action.params || {});
          return result.success ? result.output || null : result.error || null;
        },
      };

    // ── Display-only (không cần execute) ─────────────────────────
    case 'git_status':
    case 'commit_message':
      return null;

    default:
      logger.warn('[tool] No executor found for: "' + actionType + '"');
      return null;
  }
}
