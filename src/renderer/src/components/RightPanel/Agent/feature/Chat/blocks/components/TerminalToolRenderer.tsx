...
import { getPermissionDecision } from '..';
...
const TerminalToolRenderer: React.FC<TerminalToolRendererProps> = ({
  ...
  const displayCommand =
    commandText.length > 50 ? `${commandText.substring(0, 48)}...` : commandText;
    ...
    <div className="timeline-item" style={{ marginTop: '4px', paddingLeft: '29px' }}>
        ...
        >
          {isTerminalBusy && (
            <button
              ...
              style={{
                background: 'color-mix(in srgb, ' + ($('--error') || '#f44336') + ' 10%, transparent)',
                border: '1px solid color-mix(in srgb, ' + ($('--error') || '#f44336') + ' 30%, transparent)',
                cursor: 'pointer',
                color: $('--error') || '#f44336',
                padding: '4px 8px',
                borderRadius: '6px',
      ...
      {isCollapsed ? (
        ...
        <>
          ...
          />
          {needsPrompt && !isTerminalBusy && !isCompleted && (isActiveGroup || isLoading) && (
            <ExecuteButton
